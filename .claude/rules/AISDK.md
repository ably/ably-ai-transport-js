...
compiles on both. So dual support is a claim about this package's own `ai`
surface, not about the AI SDK generally: anything built on top of it — an
adapter, an example, an application — picks one major and uses that major's
spelling.
