...
The codec's one non-OpenAI output event (`function_call_output`) and the demo's own input bodies (`message` / `item` / `approval` / `regenerate`) apply as small steps onto each message's items and `toolCallStates`. A gated call's `'pending'` state has no carrier on the wire at all — the Responses API has no approval concept for a plain function call, so the merge derives it from the `function_call` item through the same `needsApproval` policy the agent gates on. The codec carries inputs as passthrough JSON typed by the application (`createResponsesCodec<OpenAIInput>()`), so the input vocabulary, the stored message model, the `/responses` flatten, and the agent loop's correlation readers are all this demo's own (`src/app/lib/openai-thread.ts`). The type parameter is an assertion, not a validated contract, so `asOpenAIInput` still validates decoded bodies at the merge boundary.
