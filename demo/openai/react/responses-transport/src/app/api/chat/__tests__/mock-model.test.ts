...
describe('createMockResponseStream', () => {
  ...
  it('streams a reasoning item before the function call for a "think ... weather" prompt', async () => {
    const events = await drain(
      createMockResponseStream({
    ...
    // The reasoning item completes before the function_call item.
    const doneItemTypes = events
      .filter((e) => e.type === 'response.output_item.done')
      .map((e) => (e.type === 'response.output_item.done' ? e.item.type : ''));
    expect(doneItemTypes).toEqual(['reasoning', 'function_call']);
    // Its summary streamed ahead of the call opening.
    expect(events.some((e) => e.type === 'response.reasoning_summary_text.done')).toBe(true);
  });
