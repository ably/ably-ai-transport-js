...
import type { OpenAIInput, OpenAITurn } from '@ably/ai-transport/openai';
...
const mockState = vi.hoisted(() => ({
  ...
  setMessages: null as null | ((messages: OpenAITurn[]) => void),
  ...
  cancel: vi.fn(async () => {}),
  send: vi.fn<(input: OpenAIInput | OpenAIInput[], opts?: SendOptions) => Promise<ClientRun<OpenAITurn>>>(),
}));
...
vi.mock('../providers', () => ({
  SessionHooks: {
    ...
    useAblyMessages: () => [],
    useView: () => {
      const [messages, setMessages] = useState<OpenAITurn[]>([]);
      useEffect(() => {
      ...
      const emptyBranch = (): BranchHandle<OpenAITurn> => ({
        hasSiblings: false,
        siblings: [],
        index: 0,
        selected: undefined,
...
const assistantTurn = (text: string): OpenAITurn => ({
  role: 'assistant',
  items: [
    {
      id: 'msg_1',
...
const clientRunStub = (): ClientRun<OpenAITurn> => ({
...
describe('<Chat>', () => {
  ...
  it('shows Stop while the latest run is actively streaming, and Stop publishes a cancel for it', async () => {
    mockState.runOf = () => ({ runId: 'run-active-1', clientId: 'user-a', status: 'active', invocationId: 'inv-1' });
