...
function MessagesTab({ messages, status }: { messages: OpenAIMessage[]; status: string }) {
  ...
  return (
    ...
    >
      ...
      {messages.length === 0 ? (
        <p className="text-xs text-zinc-700 text-center mt-8">Conversation messages will appear here as JSON.</p>
      ) : (
          ...
          <div className="flex h-16 flex-shrink-0 items-center justify-between border-b border-zinc-800 px-3">
            <div className="flex items-center gap-1">
              ...
              <button
                ...
                className={`text-[10px] px-2 py-1 rounded transition-colors ${
                  tab === 'messages' ? 'bg-zinc-800 text-zinc-300' : 'text-zinc-600 hover:text-zinc-400'
                }`}
              >
                Messages
