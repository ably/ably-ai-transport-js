...
describe('useDemoProgress', () => {
  ...
  it('drops cancel when a cancel signal is on the channel', () => {
    ...
    const { result } = renderHook(() => useDemoProgress(DEMO_SCENARIOS, [], noBranch, () => undefined, ablyMessages));
  ...
  it('drops multi-tab when turns come from more than one client', () => {
    ...
    const { result } = renderHook(() => useDemoProgress(DEMO_SCENARIOS, messages, noBranch, runOf, []));
  ...
  it('drops edit / regenerate when the matching node has siblings', () => {
    ...
    const { result } = renderHook(() => useDemoProgress(DEMO_SCENARIOS, messages, branch, () => undefined, []));
