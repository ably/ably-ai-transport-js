...
const textTurn = (role: 'user' | 'assistant', text: string): OpenAITurn => ({
  role,
  items: [
    role === 'user'
...
const paired = (turns: OpenAITurn[]): CodecMessage<OpenAITurn>[] =>
  turns.map((message, i) => ({ codecMessageId: `cm-${i}`, message }));

const noBranch: (id: string) => BranchHandle<OpenAITurn> = () => ({
  hasSiblings: false,
  siblings: [],
  index: 0,
  selected: undefined,
...
const run =
  ...
  (codecMessageId: string): RunInfo | undefined =>
    byId[codecMessageId]
      ? { runId: 'r', clientId: byId[codecMessageId], status: 'complete', invocationId: 'i' }
      : undefined;
...
describe('useDemoProgress', () => {
  ...
  it('drops edit / regenerate when the matching node has siblings', () => {
    const messages = paired([textTurn('user', 'hi'), textTurn('assistant', 'hello')]);
    const branch = (codecMessageId: string): BranchHandle<OpenAITurn> => ({
