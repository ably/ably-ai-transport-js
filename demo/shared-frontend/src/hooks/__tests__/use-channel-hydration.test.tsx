/**
 * Tests for the shared hydration pass: store-then-read ordering, the dedupe at
...
import type { HistoryMessage } from '@ably/ai-transport/vercel';
...
/** What the history read returns for a client turn: one `message` input, as the wire carried it. */
const group = (id: string): HistoryMessage => ({
  id,
  events: [{ direction: 'input', event: { kind: 'message', payload: message(id) } }],
});

const historyResult = (ids: string[], exhausted: boolean) =>
...
describe('useChannelHydration', () => {
  beforeEach(() => {
    ...
    fakeChatTransport = { readSince: historyResult([], true) };
    providerError = undefined;
  });

  it('reads the whole channel when the demo keeps no store', async () => {
    if (!fakeChatTransport) throw new Error('no adapter');
    fakeChatTransport.readSince = historyResult(['m1'], true);
  ...
  it('reads the store first and reads from the serial it reports', async () => {
    if (!fakeChatTransport) throw new Error('no adapter');
    fakeChatTransport.readSince = historyResult(['m2'], false);
  ...
  it('drops a history message the store already holds', async () => {
    if (!fakeChatTransport) throw new Error('no adapter');
    // The watermark is a lower bound, so the history read can re-return a stored turn.
    fakeChatTransport.readSince = historyResult(['m1', 'm2'], true);
    const loadStored = vi.fn().mockResolvedValue({ messages: [message('m1')], latestSerial: '01ABC@7' });
  ...
  it('reads once per adapter, even though Strict Mode runs the effect twice', async () => {
    if (!fakeChatTransport) throw new Error('no adapter');
    const { result } = renderHydration();

    await waitFor(() => expect(result.current.status).toBe('ready'));
    // A second read would advance the transport's shared history cursor past
    // the window the first one covered.
    expect(fakeChatTransport.readSince).toHaveBeenCalledTimes(1);
  });

  it('reports an error and never reads when the provider yields no adapter', async () => {
