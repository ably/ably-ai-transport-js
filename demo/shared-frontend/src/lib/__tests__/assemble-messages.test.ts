 ...
 * returns into `UIMessage`s through the provider's own reducer.
 *
...
import type { VercelInput, HistoryEvent, HistoryMessage } from '@ably/ai-transport/vercel';

import { assembleHistoryMessages } from '../assemble-messages';

const out = (event: UIMessageChunk): HistoryEvent => ({ direction: 'output', event });
const inp = (event: VercelInput): HistoryEvent => ({ direction: 'input', event });

const group = (id: string, events: HistoryEvent[]): HistoryMessage => ({ id, events });
...
describe('assembleHistoryMessages', () => {
  it('assembles an assistant turn through the reducer', async () => {
    const messages = await assembleHistoryMessages([
      group('ui-1', [
  ...
  it('contains a reducer failure to the one message', async () => {
    ...
    try {
      const messages = await assembleHistoryMessages([
        ...
        // provider's business; either way the rest of the history read must arrive.
