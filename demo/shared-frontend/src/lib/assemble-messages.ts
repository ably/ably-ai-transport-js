/**
 * Turn the groups `readSince` returns from the channel into the `UIMessage`s
 * `useChat` starts from.
 *
 ...
 *
 * One reducer call per group, never one for the whole read. The reducer holds
...
async function assembleGroup(group: HistoryMessage): Promise<UIMessage | undefined> {
  ...
  for (const historyEvent of group.events) {
    if (historyEvent.direction === 'output') {
      chunks.push(historyEvent.event);
      continue;
    }
    const input = historyEvent.event;
    switch (input.kind) {
      case 'message': {
        clientTurn =
  ...
  for (const decision of decisions) {
    ...
    // the history output already carries the answer to.
...
/**
 * Assemble every group the history read returned.
 * @param groups - The history groups, oldest first.
 * @returns The messages, oldest first, skipping any group that produced none.
 */
export async function assembleHistoryMessages(groups: readonly HistoryMessage[]): Promise<UIMessage[]> {
  ...
  for (const group of groups) {
    ...
    try {
      ...
      console.warn('assembleHistoryMessages(); dropping a message the reducer rejected', group.id, error);
    }
  }
  return messages;
}
