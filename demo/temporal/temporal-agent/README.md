...
- **The SDK's Temporal plugin** (`createAblyTransportPlugin`, registered in `src/worker/index.ts` with the options shared in `src/worker/ably.ts`) supplies the run's framing activities — `openRun`, `endRun`, `cleanupRun` — so none of them appear in this demo's code. The workflow drives them through `withRun` from `@ably/ai-transport/temporal/workflow`, which opens the run and makes a best-effort attempt to close it if the turn fails.
- **Durable re-entry through the SDK's activity helpers.** Each of the demo's own activities runs inside `withStep` from `@ably/ai-transport/temporal`, bound in `src/worker/ably.ts` to the same codec, client factory and logger the plugin uses. It builds a fresh Ably client and agent transport, re-enters the open run without publishing, and closes both afterwards — so nothing reaches the wire until the activity publishes output or a terminal.
...
- **Retry supersession**: `withStep` keys each activity's step on the Temporal activity id, which is stable across retries of the same activity, so a fresh-process retry's channel output supersedes the failed attempt's instead of appending beside it.
...
- **Client tool** — _"What's the weather?"_ (no location) — `getLocation` has no `execute` on the server; the inference activity publishes `ai-run-end` inline and the workflow returns. The browser executes `navigator.geolocation`, publishes the result on the channel, POSTs a continuation. A fresh workflow answers on a new run.
- **Approval** — _"What's the weather forecast for tomorrow in London?"_ — the model calls `getWeatherForecast`, which requires approval. The workflow ends the turn. Approve in the UI, and the continuation POST starts a fresh workflow that runs the tool.
