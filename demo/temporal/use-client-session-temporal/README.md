...
- One Temporal activity per SDK step, with the pairing handled for you: `ablyTransport.activity(...)` adopts the run and opens a step under a `stepIdFor`-derived id, which composes the run's invocation id with the current Temporal `activityId`.
- **The SDK's Temporal plugin** (built in `src/worker/ably-transport.ts`, registered in `src/worker/index.ts`) supplies the run's framing activities — `openRun`, `endRun`, `suspendRun`, `cleanupRun` — so none of them appear in this demo's code. The workflow drives them through `withRun` from `@ably/ai-transport/temporal/workflow`, which opens the run (creating it, or resuming it when the turn is a continuation) and makes a best-effort attempt to close it if the turn fails.
- **`ablyTransport.activity(...)`** wraps this app's own two activities, so neither builds an Ably client, connects a session, adopts the run, opens a step or tears any of it down. `runToolStep` adds nothing but the tool call. `runInferenceStep` passes `history: 'full'` for the conversation the model needs, runs its steering loop into the one step it was handed, and closes it with `finishStep` so a cancelled turn reads `cancelled`.
- **Pooled connections**: the plugin holds Ably connections for the life of the worker, so a turn's activities share one WebSocket instead of opening one each.
- **`finishRun` / `finishStep`** (from `@ably/ai-transport/vercel`) route a `VercelRunOutcome` to the right terminal call, so the demo carries no outcome-to-SDK translation of its own.
- Two activities are the app's own: one `runInferenceStep` per LLM call and one `runToolStep` per server tool. The activity that reaches a terminal outcome publishes it (`ai-run-suspend` / `ai-run-end`) inline before returning, which is free because it already has the run loaded.
...
- Cancellation from Temporal's side, which is a separate path: `temporal workflow cancel` reaches the running activity because it heartbeats and declares a `heartbeatTimeout` in `src/worker/workflows.ts`. Without both, Temporal has no way to tell the activity, since it reports a cancel only in the response to a heartbeat.
