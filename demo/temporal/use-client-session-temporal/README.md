...
- One Temporal activity per SDK step: `openRun` (create + start the run), one `runInferenceStep` per LLM call, one `runToolStep` per server tool, and a workflow-level `cleanupRun`. The activity that reaches a terminal outcome publishes it (`ai-run-suspend` / `ai-run-end`) inline before returning — there is no separate suspend/end activity.
