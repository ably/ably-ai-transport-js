 ...
 * `stripToolExecutes`, `pendingToolCalls`, `stepIdFor`,
 * `activityInvocationIdFor`, `step.send` — lives in the SDK across three
 * subpaths:
 *
 ...
 *   - `@ably/ai-transport/temporal` — stepIdFor, activityInvocationIdFor
 *
...
import { activityInvocationIdFor, stepIdFor } from '@ably/ai-transport/temporal';
...
export async function openRun(input: { invocation: InvocationData; invocationId: string }): Promise<OpenRunResult> {
  ...
  try {
    ...
    const outcome = await _runInferenceStep(run, stepIdFor(input.invocationId, activityId));
...
export async function runInferenceStep(input: StepInput): Promise<InferenceOutcome> {
  ...
  try {
    ...
    const run = session.adoptRun(
      { runId: input.ids.runId, invocationId: activityInvocationIdFor(), triggerEventId: input.ids.triggerEventId },
      { signal: cancelSignal },
    );
    ...
    const outcome = await _runInferenceStep(run, stepIdFor(input.ids.invocationId, activityId));
...
export async function runToolStep(input: StepInput & { toolCall: ToolCallInfo }): Promise<void> {
  const activityId = Context.current().info.activityId;
  const stepId = stepIdFor(input.ids.invocationId, activityId);
  const cancelSignal = Context.current().cancellationSignal;
  const ably = makeAbly();
