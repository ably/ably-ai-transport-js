...
export async function GET(req: Request) {
  ...
  // The game state lives in LiveObjects, so the token must also permit the
  // object operations — without them the server grants the channel's object
  // modes as an empty subset and object reads/writes fail.
  const namespace = process.env.NEXT_PUBLIC_ABLY_CHANNEL_NAMESPACE ?? 'ai:';
  const capability = ['publish', 'subscribe', 'history', 'object-subscribe', 'object-publish'];
