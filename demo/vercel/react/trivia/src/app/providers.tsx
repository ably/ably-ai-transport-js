...
export function Providers({ clientId, children }: { clientId?: string; children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    // `NEXT_PUBLIC_ABLY_ENDPOINT` lets tests point the browser client at the
    // Ably sandbox (`nonprod:sandbox`); unset in normal use, so it defaults
    ...
    const ably = new Ably.Realtime({
      ...
      // The game state lives in LiveObjects, which is an ably-js plugin —
