...
const config = [
  ...
  ...typescript,
  {
    ...
    // connects the session on mount and appends a transition entry whenever
    // useChat's status changes. The React-Compiler rule
    // react-hooks/set-state-in-effect can't tell these apart from accidental
    // cascading setState, and a lazy useState initialiser can't run the client
    // teardown. Scope the rule off to just these files rather than suppress
    // inline.
