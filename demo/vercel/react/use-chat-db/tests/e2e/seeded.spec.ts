...
// seeds `useChat({ messages })` from it on load. `useMessageSync` then walks the
// live channel back to the seam (the newest seed message) and composes
// seed ⧺ live with no duplicate. This proves the reconciliation over real Ably
// history across a page reload.
