...
- **Gate before publishing** — every re-entering activity folds the run's lifecycle events from history first (latest event per run) and publishes only while the run is still this workflow's to drive. A run that ended, suspended, or was resumed by another invocation (a client continuation) is left alone, which is what replaces the old session-level status gating.
...
- **The terminal is its own activity, gated.** The inference classifies its outcome and publishes output only; a separate `terminal` activity folds the run's lifecycle from history and publishes `ai-run-suspend` / `ai-run-end` only while the run is still this workflow's to drive. A process that dies between outcome and terminal is retried at the terminal alone.
