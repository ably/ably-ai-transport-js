...
- **A driver-owned agent loop** — the workflow runs one activity per unit of work: `open` (locate the trigger, publish the run's opening event), an `inference` per model call, one `tool` per server tool call, and a `terminal` that publishes the run's end — looping until the turn settles. `stripToolExecutes` stops the model from running tools inline, so the workflow owns the loop.
- **Message assembly from the channel** — each activity folds channel history into `UIMessage[]` with the AI SDK's own reducer (`readUIMessageStream`) via the demo's `lib/fold-messages.ts`: outputs and tool-resolution inputs fold as chunks, whole-message inputs merge per codec-message-id, approvals apply as synthesized approval-response chunks, and only the canonical attempt per AIT step folds.
- **Gate before publishing** — every re-entering activity folds the run's lifecycle events from history first (latest event per run) and publishes only while the run is still this workflow's to drive. A run another invocation already ended or resumed (a client continuation) is left alone.
- **Continuations across processes** — a client tool or an approval still ends the turn (`run.end({ reason: 'complete' })` from the terminal activity), because the useChat adapter publishes each resolution as a plain input carrying no run id. The continuation POST starts a fresh workflow that opens a new run.
...
- **Client tool** — _"What's the weather like?"_ — `getLocation` has no server `execute`; the turn ends, your browser resolves the location (geolocation prompt), and a fresh workflow answers.
- **Approval** — _"What's the weather forecast for London?"_, then **Approve** — `getWeatherForecast` is approval-gated; the turn ends until you decide, then a fresh workflow runs the tool.
...
- **Durability boundary.** An LLM stream isn't resumable mid-chunk; WDK's durability applies at the activity boundary — a failed activity is retried in full under the same WDK step id, and the retried attempt's output supersedes the dead attempt's in the durable record. The response-message id is pinned per AIT step (`toUIMessageStream({ generateMessageId })`) so a folding consumer keys both attempts to one message. A client that already rendered a dead attempt's partial chunks live keeps them until reload — the useChat stream is append-only.
