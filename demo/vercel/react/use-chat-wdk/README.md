...
- **Gate before publishing** — every re-entering activity folds the run's lifecycle events from history first (latest event per run) and publishes only while the run is still this workflow's to drive. A run that ended, suspended, or was resumed by another invocation (a client continuation) is left alone.
