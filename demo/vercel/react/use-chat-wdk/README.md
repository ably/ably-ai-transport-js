...
- **Gate before publishing** — every re-entering activity reads the run's lifecycle events from history first (latest event per run) and publishes only while the run is still this workflow's to drive. A run another invocation already ended or resumed (a client continuation) is left alone.
