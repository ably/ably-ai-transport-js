...
import { generateClientName } from '@ably-ai-demos/frontend';
...
export async function GET(req: Request) {
  ...
  // Least-privilege caps: publish/subscribe cover the chat channel and the
  // sidecar `<channel>:wdk` activity-telemetry channel (both under `<namespace>*`),
  // presence backs the avatar stack, history backs conversation hydration.
  const capability = ['publish', 'subscribe', 'presence', 'history'];
