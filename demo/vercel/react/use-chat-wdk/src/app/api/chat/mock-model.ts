...
/** Pull a place name out of a "... in/for/at <place>?" prompt, if one is named. */
function namedLocation(text: string): string | undefined {
  const match = /\b(?:in|for|at)\s+([A-Za-z][A-Za-z .,'-]*?)\s*[?.!]?$/.exec(text.trim());
  return match ? match[1].trim() : undefined;
}

/** A named place, or London when the prompt named none. */
function extractLocation(text: string): string {
  return namedLocation(text) ?? 'London, UK';
}
...
function planResponse(prompt: ModelPrompt): ResponsePlan {
  ...
  if (lower.includes('weather')) {
    ...
    const place = namedLocation(text);
    if (place) return { kind: 'tool', toolName: 'getWeather', input: { location: place } };
...
/**
 * Mirror a real provider's rejection of a prompt that ends with an assistant
 * message (Anthropic treats a trailing assistant message as a prefill and
 * rejects the request). No prompt this demo builds should legitimately end with
 * one — a fresh turn ends with the user input and a tool continuation converts
 * to a trailing `tool` message — so hitting this means the prompt leaked the
 * run's own in-flight reply (e.g. a durable retry re-reading its dead attempt's
 * output), which the real model would reject too.
 */
function trailingAssistantError(): ModelStream {
  return new ReadableStream({
    start(controller) {
      controller.enqueue({ type: 'stream-start', warnings: [] });
      controller.error(new Error('mock provider: prompt must not end with an assistant message (prefill)'));
    },
  });
}
      ...
      Promise.resolve({
        stream:
          options.prompt.at(-1)?.role === 'assistant'
            ? trailingAssistantError()
            : buildStream(planResponse(options.prompt), options.abortSignal),
      }),
  });
}
