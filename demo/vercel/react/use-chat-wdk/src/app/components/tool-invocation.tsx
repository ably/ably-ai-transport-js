...
function LocationResult({ output }: { output: unknown }) {
  // CAST: a tool part's output is untyped; treat it as getLocation's declared
  // outputSchema shape (a trust boundary — the tool owns that schema).
...
function ToolApprovalCard({
  ...
  // CAST: a tool part's input is untyped; render its values as a plain record.
  ...
  switch (part.state) {
    ...
    case 'output-available': {
      if (getToolName(part) === 'getWeather') {
        // CAST: untyped tool output — getWeather's execute returns WeatherData.
        return <WeatherCard data={part.output as WeatherData} />;
      }
      if (getToolName(part) === 'getWeatherForecast') {
        // CAST: untyped tool output — getWeatherForecast's execute returns ForecastData.
