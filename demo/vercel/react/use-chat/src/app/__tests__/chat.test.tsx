...
// identity, so a fresh literal per render would re-run the history read forever.
...
describe('<Chat>', () => {
  ...
  it('seeds from the store and appends what the channel read returns', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn(() =>
        Promise.resolve(
    ...
    // readSince hands back the events it read; the hydration hook assembles
      ...
      messages: [
        {
          ...
          events: [
            ...
            { direction: 'output', event: { type: 'text-delta', id: 't1', delta: 'history reply' } },
    ...
    expect(screen.queryByText('history reply')).not.toBeNull();
  });

  it('reads the channel back only as far as the serial the store reports', async () => {
    vi.stubGlobal(
      'fetch',
  ...
  it('reads the whole channel when the store reports no serial', async () => {
    render(<Chat chatId="ai:test" />);
  ...
  it('resumes once after the history read, so a run still streaming is picked up', async () => {
    render(<Chat chatId="ai:test" />);

    // Step five of the hydration flow. The history read has already run, so the
