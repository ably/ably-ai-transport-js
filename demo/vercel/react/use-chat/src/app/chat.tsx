 ...
 * `chatTransport.readSince(latestSerial)` reads the channel back only as far
 ...
 * seam.
 *
 * The history read is what makes a run still streaming resumable. `readSince`
 ...
 * fetch and the history read have run, and a reconnect with nothing retained has no
 * run to resume.
 */
...
function ChatInner({
  ...
  // conversation (the store plus the history read), and the adapter turns its sends
  ...
  // Step five of hydration, once per mount: pick up whatever the history read
  // withheld. The history read has already run by the time this component mounts, so
  ...
  // adapter fires this only while idle, so it cannot fight our own send, which
  // means a run that started while we were streaming is announced later — when
  // we go idle — and is already part-way through by then.
