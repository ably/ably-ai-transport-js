 ...
 * which reads the channel back only as far as it and returns whatever was
 ...
 * transport's wire metadata in the application's schema, and the history read recovers
...
export interface StoredConversation {
   ...
   * accounted for in `messages`, which is what lets a client read back only as
   * far as it.
   */
  latestSerial?: string;
}
