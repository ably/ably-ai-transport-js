...
export function InputBar({ session, send, activeRuns, clientId, queue, placeholder }: InputBarProps) {
  ...
  const hasOwnRuns = clientId ? activeRuns.has(clientId) : false;

  const slash = useSlashCommands(session, activeRuns, send, input);
  ...
  const handleSubmit = useCallback(() => {
    ...
    // If own runs are active, queue the message
    if (hasOwnRuns) {
      queue.add(text);
    } else {
  ...
  const buttonLabel = isSlash ? 'Run' : hasOwnRuns ? 'Queue' : 'Send';
    ...
    <div className="border-t border-zinc-800 px-4 py-3">
      <div className="relative flex gap-2">
        ...
        <input
          ...
          autoFocus
        />
        {hasOwnRuns && !input.startsWith('/') && (
          <button
            type="button"
            onClick={() => session.cancel({ own: true })}
