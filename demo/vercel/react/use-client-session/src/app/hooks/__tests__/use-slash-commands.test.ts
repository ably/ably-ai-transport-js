...
describe('useSlashCommands', () => {
  ...
  it('execute /cancel <runId> calls transport.cancel with runId', () => {
    const transport = createMockTransport();
    const send = createMockSend();
    ...
    expect(executed).toBe(true);
    expect(transport.cancel).toHaveBeenCalledWith({ runId: 'abc123' });
  });
  ...
  it('shows active run IDs in suggestions', () => {
    ...
    const { result } = renderHook(() => useSlashCommands(transport, activeRuns, send, '/'));
