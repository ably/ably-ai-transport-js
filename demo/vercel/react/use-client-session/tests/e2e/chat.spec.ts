...
test.describe('use-client-session demo - chat behaviour', () => {
  ...
  // test.fixme: client-session-specific (passes in use-chat) - the regen group
  // counter is not rendered in this flow. Remove `.fixme` once the View handles it.
  // checks: interleaved edit + regen; each operation keeps its own local 2/2 group.
  test.fixme('exploratory: interleaved edit and regenerate keep independent branch groups', async ({
    page,
  }, testInfo) => {
    await page.goto(freshChannelUrl(testInfo.title));
