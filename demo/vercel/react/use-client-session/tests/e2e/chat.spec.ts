...
test.describe('use-client-session demo - chat behaviour', () => {
  ...
  test('client-side tool: getLocation executes and its output appears in the visible bubble', async ({
    ...
    try {
      ...
      // The getLocation tool part must render in the View. This demo folds the
      // result optimistically via view.send, so the bubble can flip from
      // "Calling getLocation" (input-available) straight to the "Location:" card
      // (output-available) within a frame; accept either.
      const toolRendered = page.locator('text=/Calling getLocation|Location:/').first();
      await expect(toolRendered).toBeVisible({ timeout: 60_000 });
  ...
  test('exploratory: interleaved edit and regenerate keep independent branch groups', async ({ page }, testInfo) => {
    // The branch counter (span.tabular-nums) re-renders a tick AFTER
    // waitForAssistantSettled returns: in this demo the "run settled" signal
    // (runOf(last).status) and the View's branch-counter re-convergence land on
    // different React commits, so a one-shot read can observe a transient null.
    // Poll each counter assertion so we measure the converged state, not the
    // first frame after settle.
    await page.goto(freshChannelUrl(testInfo.title));
  ...
  // test.fixme: flaky - "Load older" history hydration after a refresh
  // intermittently surfaces only the newest run. Remove `.fixme` once reliable.
  // checks: "Load older" works after a refresh.
  test.fixme('exploratory: pagination — Load older messages button is functional after a refresh', async ({
    page,
  }, testInfo) => {
