...
- **`inputClientId`** — the clientId of the input event currently driving the agent. The agent reads it from the publisher's Ably `clientId` on the triggering `ai-input` and re-stamps it on its own publishes. May differ from `runClientId` when a re-entering (resume) continuation is triggered by an input from a non-owner (e.g. another client publishes an approval response for someone else's run).

For a fresh run the two are equal. They diverge on a **re-entering** continuation: imagine Client A starts a run that suspends awaiting a tool approval; Client B then publishes an approval-response `ai-input` against that run and POSTs the continuation. The agent's `ai-run-resume` carries `runClientId: A` (still A's run) and `inputClientId: B` (B published the input that drove this invocation). Every assistant output published during B's invocation carries the same pair.

A client **tool result** behaves differently: it does not re-enter the suspended run but [forks](branching.md#client-tool-results) its own reply run — published run-less, its run-id minted by the agent — owned by the publishing client. So concurrent tool results from clients — including two tabs sharing one `clientId` — land on segregated sibling runs rather than converging on the suspended run, and each fork run carries its publisher as **both** `runClientId` and `inputClientId`.
