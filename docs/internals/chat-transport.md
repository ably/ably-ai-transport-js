...
| `submit-message`, last is assistant in tree (continuation) | None (empty array)    | Entire array         | A `tool-result` **forks** a fresh reply run — published RUN-LESS (no `runId`; `parent` = the suspended run's input node, `role: 'assistant'`), the agent mints the fork's `runId`; an approval response re-enters the suspended assistant's `runId` (or observes that run when there are no inputs to send, see below) |
| `regenerate-message`                                       | None (empty array)    | Entire array         | Derived by `View.regenerate` from the tree, not precomputed here                                                                                                                                                                                                                                                       |

For **edit** and **fork-on-unresolved-tool**, the adapter looks up the target in the [conversation tree](conversation-tree.md) (via `view.getMessages()`, whose entries each carry a `codecMessageId`) to compute `forkOf` and `parent` using the tree's `codec-message-id` (not the `UIMessage.id`). For **regeneration**, the adapter does NOT precompute fork metadata — it routes the target's codec-message-id through `view.regenerate()`, which derives `forkOf`/`parent` from the tree itself.

A **continuation** (a `submit-message` whose last message is an assistant already in the tree) covers useChat's auto-submit after a tool result and multi-step tool use. It publishes no new message; instead the adapter walks useChat's overlay against the tree and synthesizes the `tool-result` / `tool-result-error` / `tool-approval-response` inputs for any tool parts the user resolved, then sends them via `view.send`. A `tool-result` / `tool-result-error` **forks** its own reply run — published RUN-LESS (no `runId`), parented at the suspended run's input node with `role: 'assistant'`; the agent mints the fork's `runId` and opens it with `ai-run-start`, and the tree reconciles the client's optimistic reply run onto that id by the tool-result's codec-message-id — so concurrent results for one tool call stay segregated on sibling branches. A `tool-approval-response` (a non-tool-result continuation) instead re-enters the suspended assistant's `runId` via `ai-run-resume`.

If that walk yields **no** inputs — every resolution the overlay holds is already folded into the tree, e.g. because another tab with the same `clientId` published its result first and this client received it before auto-submitting — there is nothing to send. The adapter then skips both `view.send` (the core rejects an empty send) and the invocation POST, and instead returns a deferred observe-only stream (see [Stream return](#stream-return)) keyed on the suspended assistant's `runId`. That stream keeps `useChat` in `streaming` — suppressing an auto-resubmit loop and holding the `useMessageSync` gate shut — until the run the other tab is driving produces its next turn, at which point it closes and `useMessageSync` repaints the overlay from the tree (now carrying the other tab's result and the agent's reply). The other tab owns waking the agent, so this client never POSTs.
...
The empty-continuation case (above) returns a different stream from the same module: `createDeferredContinuationStream`. It forwards **no** chunks — its sole job is to keep `useChat` in `streaming` until the run the other tab is driving produces its **next assistant turn**, then close so `useMessageSync` repaints from the tree. Holding `streaming` until that turn appears is the point: it stops `useChat` re-evaluating `sendAutomaticallyWhen` and resubmitting the same empty continuation in a loop — once a new assistant message follows the resolved tool call, that condition is no longer met. The first `output` for the run is the start of that turn; `run-end` is the safety net. It closes on the first of: a synchronous snapshot showing the run is missing or no longer in a live state (`active`/`suspended`) — closing the race where the run already ended before this stream subscribed; the run's next `output`; its `run-end`; or a `DEFERRED_CONTINUATION_TIMEOUT_MS` best-effort floor. The timeout `close()`s rather than errors, so the user sees no failure: a run whose continuation is never driven re-polls once per timeout (the clean close reopens the gate, `sendAutomaticallyWhen` is still satisfied, and `useChat` resubmits) instead of hanging a single stream forever — and if the agent was merely slow, the next cycle picks up the reply. It errors only on a session `error`, like `createRunOutputStream`. Both factories share one settle-once scaffold (`createSettlingStream`).
