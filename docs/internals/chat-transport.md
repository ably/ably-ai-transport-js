...
| `submit-message`, last is assistant in tree (continuation) | None (empty array)    | Entire array         | None; reuses the assistant's `runId` (or observes that run when there are no inputs to send, see below) |
