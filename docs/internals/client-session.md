...
1. **Generate identifiers** — per-message `codecMessageId`s and `inputEventId`s (all `crypto.randomUUID()`). Neither the `runId` nor the `invocationId` is minted here. The agent mints the `invocationId` per HTTP request, and the `runId` too for a fresh run; a continuation carries a run-id via `options.runId` — the existing run's id for a plain re-entry (a resume), or a fresh forked id (with a `parent`) for a client tool-result fork — which the client stamps as-is on the continuation's `ai-input` (the agent reads it off the channel, not from the invocation body).
