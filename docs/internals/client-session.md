...
6. **POST in parallel** — the HTTP POST is fired in parallel with the publish. The body carries `runId`, `invocationId`, `eventIds`, and `history` (the View's pre-computed flat TMessage[]) plus optional `body` extras. Per-message metadata (`clientId`, `parent`, `forkOf`, `isContinuation`) lives on channel headers, not in the POST body.
...
8. **Return `ActiveRun`** — once run-start arrives, the caller receives `{ stream, runId, invocationId, cancel(), optimisticCodecMessageIds, eventIds }`.
