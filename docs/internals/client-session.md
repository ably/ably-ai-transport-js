...
When `send()` receives multiple inputs, it chains them into a linear thread: each input after the first uses the previous one's `codec-message-id` as its `parent`. This produces a connected sequence rather than siblings at the same fork point. Chaining only applies to inputs that fold optimistically (a wire-only input such as `regenerate` does not advance the auto-parent).
...
2. Clears the session's `error` handlers, closes all views, and rejects any unresolved `ActiveRun.runId` promises with `SessionClosed`
