...
### Mutable message **(Ably)**

An Ably message that can be modified after publication. A mutable message starts with a `publish` (which returns a serial), then receives `appendMessage` calls that add data, and ends with a closing append that sets the final state. The [encoder](encoder.md) uses mutable messages to stream LLM tokens — each token is appended to a single persistent message rather than published as a separate message.

The alternative is a **discrete message** — a single immutable publish. User messages, tool output, and lifecycle events are discrete.
...
| `message.append` | Data was appended to an existing mutable message    |
...
- **Transport layer** — generic machinery shared by all codecs. Handles turn lifecycle, stream routing, echo detection, cancel signals, and conversation tree management. Uses `x-ably-*` headers. Lives in `src/core/transport/`.
...
- **Message ID** (`x-ably-msg-id`) — uniquely identifies a single domain message (a `crypto.randomUUID()` generated by the client or server transport). Used for [echo detection](wire-protocol.md#echo-detection), [accumulator routing](codec-interface.md#accumulator), and [conversation tree](conversation-tree.md) node identity. For streamed messages, every append carries the same msg-id so the entire mutable message lifecycle shares one identity.
...
When a client calls `send()`, it inserts an optimistic message into the conversation tree (with no serial). The server then publishes that message to the channel, and the client receives it back. The client matches the echo by `x-ably-msg-id` and updates the optimistic entry with the server-assigned serial ([serial promotion](conversation-tree.md#upsert-the-sole-mutation)) rather than creating a duplicate.
