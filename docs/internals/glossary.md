...
- **Message ID** (`x-ably-msg-id`) — uniquely identifies a single domain message (a `crypto.randomUUID()` generated by the client or server transport). Used for [echo detection](wire-protocol.md#echo-detection), [accumulator routing](codec-interface.md#accumulator), and [conversation tree](conversation-tree.md) node identity. For streamed messages, every append carries the same msg-id so the entire message append lifecycle shares one identity.
