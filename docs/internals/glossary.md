...
The shared read surface every run exposes, on the client and the agent alike (`BaseRun`, `src/core/transport/base-run.ts`): `runId`, `status` (`active` / `suspended` / `complete` / `cancelled` / `error`), `error` (set only when `status` is `error`), and `messages` - **this run's whole contribution**, its originating input plus all of its output across every suspend/resume segment (including tool results), not the whole conversation. The fields are live getter accessors over the [conversation tree](conversation-tree.md), not a snapshot, so they stay current as lifecycle events fold in. The client's `ClientRun` extends it with the send/cancel handle and the `started` promise (`runId` stays a string, populated once `started` resolves); the agent's `AgentRun` extends it with [`run.view`](agent-session.md#run-view) and [`run.located`](agent-session.md#runlocated). For the full conversation (not just this run) drain `run.view`.
...
- **LeafBranchSource** — the `BranchSource` behind the agent's `run.view`. It pins to a single branch by walking `parentCodecMessageId` edges back from the run's leaf - no sibling selection map, branch choice is implicit-by-parent-walk. That structural branch is the full ancestor chain of its triggering input; the prompt projection it exposes (`visibleNodes` / `getMessages`) additionally omits ancestor runs that haven't completed, along with the inputs they replied to, to keep a dangling tool call out of the model prompt. The current run is always kept.
