...
- **Turn ID** (`x-ably-turn-id`) - groups all messages in one request-response cycle. A single turn may produce multiple messages (user message, assistant text, lifecycle events). Used for cancellation scope, active turn tracking, and stream routing.
