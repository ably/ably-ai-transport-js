...
Messages with `amend` are skipped - amendments target an existing message rather than producing a new completion. Messages without `codec-message-id` (run lifecycle events) are skipped too. `message.delete` is never a start signal: it clears the decoder's tracker and emits nothing.
...
Messages are grouped by [`run-id`](wire-protocol.md#transport-headers). Each run gets its own [`MessageAccumulator`](codec-interface.md#accumulator) instance. Messages without a run IDs go to a default accumulator.
...
Each `HistoryItem` pairs a decoded message with its canonical [transport headers](wire-protocol.md#transport-headers) and Ably serial. The session uses these to seed the [conversation tree](conversation-tree.md#upsert-the-sole-mutation) with correct [branching metadata](wire-protocol.md#branching-headers) and serials.
...
- **Per-run headers by codec-message-id** - the last-seen headers for each [`codec-message-id`](wire-protocol.md#message-identity-codec-message-id) within a run (closing appends override earlier headers, e.g. [status](wire-protocol.md#streamed-messages) changes from `"streaming"` to `"complete"`)
