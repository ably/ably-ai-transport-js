...
| ------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- |
| Tree structural change (new node, deletion, serial promotion) | `_onTreeUpdate()` calls `recomputeAndEmitIfChanged()`                                                                             |
...
| Fork / regenerate auto-selection after a write                | `branchSource.applyForkAutoSelect()` / `applyRegenerateAutoSelect()` trigger `recomputeAndEmit()` / `recomputeAndEmitIfChanged()` |
| History page revealed                                         | `_releaseWithheld()` calls `recomputeAndEmit()`                                                                                   |
