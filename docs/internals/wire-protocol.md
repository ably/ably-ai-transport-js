...
| `run-id`                 | string                                       | [Run](glossary.md#run-id-vs-message-id) correlation ID. Every message in a run carries this                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| `invocation-id`          | string                                       | Per-invocation correlator. Stamped on every client-published event in a send (user-message AND amend events) and echoed on `run-start` / `run-end`. Surfaced on the run's `RunNode` and message metadata; it is **no longer** the handle the client uses to match a `run-start` to its pending send — see `input-codec-message-id`                                                                                                                                                                               |
...
| `run-continue`           | `"true"`                                     | Marks a client `ai-input` as a continuation of an already-started run rather than the input that opens a fresh run. The agent reads it off the triggering input to decide whether to open the run with `ai-run-start` or re-enter it with `ai-run-resume`; it is not echoed onto the agent's lifecycle events                                                                                                                                                                                                    |
...
interface InvocationData {
  ...
  invocationId: string;
  sessionName: string;
  // …history, inputEventIds, etc.
}
```

The body does **not** carry a `clientId` field. The agent reads the input event off the channel (matched via `inputEventIds`) and takes its `clientId` from the publisher's Ably-level `clientId` on the wire message.
...
| ---------------- | ---------------- | --------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| `ai-run-start`   | Server → Channel | `run-id`, `run-client-id`               | `parent`, `fork-of`, `invocation-id`, `input-client-id`, `input-codec-message-id`                                       | Signal that a run has started (its first invocation)                                                                      |
...
| `ai-run-resume`  | Server → Channel | `run-id`, `run-client-id`               | `invocation-id`, `input-client-id`, `input-codec-message-id`                                                            | Signal that a subsequent invocation re-entered an existing run (a continuation); carries no `parent` / `fork-of`          |
...
When a client calls `send()`, it inserts an optimistic message into the conversation tree (with no serial). The server then relays that message onto the channel. When the client receives the relay it matches by `codec-message-id` — via the tree's `codec-message-id → run-id` index — and reconciles the optimistic entry with the server-assigned serial ([serial promotion](conversation-tree.md#upsert-the-sole-mutation)) rather than creating a duplicate. The match keys on `codec-message-id`, **not** the `run-id`: an echo reconciles to its optimistic node even when the wire `run-id` differs, which keeps reconciliation correct as `run-id`s move to agent minting.
...
| `run-start`             | Agent (`RunManager.startRun`) | `invocation.parent` from the POST body (= the client's `autoParent`). `run-start` only ever opens a fresh run; a continuation re-enters via `ai-run-resume`, which carries no `parent`                                                                                             |
