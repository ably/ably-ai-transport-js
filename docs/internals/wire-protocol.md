...
| `run-start`             | Agent (`RunManager.startRun`) | The same value the agent resolves for its assistant output (next row). `run-start` only ever opens a fresh run; a continuation re-enters via `ai-run-resume`, which carries no `parent`                                                 |
