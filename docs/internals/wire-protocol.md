...
| `run-id`                 | string                                                   | [Run](glossary.md#run-id-vs-message-id) correlation ID, **minted by the agent**. Carried by every agent-published message and by a client continuation (the reused id). A fresh client `ai-input` omits it — the agent assigns the run id at `run-start` and the client adopts it; until then the client keys the run by the triggering `codec-message-id`                                                                                                                                                        |
...
| `invocation-id`          | string                                                   | Per-invocation correlator, **minted by the agent** (its POST handler). Echoed on `run-start` / `run-end` and surfaced on the run's `RunNode` and message metadata. The client no longer stamps it on its published `ai-input` events, nor uses it to match a `run-start` to its pending send — see `input-codec-message-id`                                                                                                                                                                                       |
...
| `input-codec-message-id` | string                                                   | The `codec-message-id` of the input event that triggered the current invocation (the one whose `event-id` matched the invocation's `inputEventId`). The agent re-stamps it on every event it publishes for the invocation (run lifecycle + outputs), mirroring `input-client-id`. The client correlates a `run-start` to its pending send — and reconciles [`ActiveRun.started`](client-session.md) — by this id, since it is the only identity the client owns at send time now that `run-id`s are agent-minted. |
| `event-id`               | string                                                   | Per-event identifier on each client-published event in a send. The invocation body lists every inputEventId; the agent's input-event lookup waits for all of them on the channel before starting LLM work                                                                                                                                                                                                                                                                                                         |
| `run-continue`           | `"true"`                                                 | Marks a `run-start` as a continuation of an already-started run rather than the first start of that `runId`                                                                                                                                                                                                                                                                                                                                                                                                       |
| `run-reason`             | `"complete"` / `"cancelled"` / `"error"` / `"suspended"` | Why a run ended (on run-end events). `suspended` signals the run is paused awaiting a continuation invocation (e.g. tool approval)                                                                                                                                                                                                                                                                                                                                                                                |
| `error-code`             | numeric string                                           | Set on `ai-run-end` when `run-reason: error`. Numeric `Ably.ErrorInfo` code. The client derives `statusCode` from this — `Math.floor(code / 100)` for codes in `10000–59999`, else `500`                                                                                                                                                                                                                                                                                                                          |
| `error-message`          | string                                                   | Set on `ai-run-end` when `run-reason: error`. Human-readable error message                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| `finishReason`     | Why the LLM stopped generating                    |
| `error`            | Error message                                     |
| `data`             | JSON-serialized data payload (for `data-*` parts) |
...
| `input-client-id` | Per invocation | every published event | Constant within one invocation; updates on a continuation `ai-run-start` if a different client published the input that triggered it. | "Whose input is driving the agent right now?" |

The agent reads the triggering `ai-input` event off the channel (matched by `event-id`) and takes the publisher's Ably-level `clientId` directly off that wire message. It then re-stamps that value as `input-client-id` on every event it publishes for the invocation — `ai-run-start`, `ai-run-end`, every assistant output, every `addMessages`/`addEvents` publish. By the same mechanism it stamps `input-codec-message-id` (the triggering input's `codec-message-id`) on those same events, giving the client a reconciliation handle it owns at send time — see [Optimistic reconciliation](#optimistic-reconciliation). The run owner's `clientId` is stamped as `run-client-id` on the same events. For a fresh run the two are equal; on a continuation invocation triggered by an input event from a non-owner (e.g. a tool-result publish from a different client), `input-client-id` reflects that other client while `run-client-id` stays put.
...
The HTTP POST body the client sends to the agent endpoint carries only the client-owned identifiers the agent needs out-of-band before the channel is observable — a pointer to the input event the agent should look up, and the channel name:

```ts
interface InvocationData {
  inputEventId: string;
  sessionName: string;
  runId?: string; // present only for a continuation (the reused, already-adopted id)
  invocationId?: string; // omitted by the client — the agent's POST handler mints it
}
```

Run and invocation identity are the agent's to mint: a fresh-run body omits both `runId` and `invocationId`, and the agent's POST handler mints the invocation id (returned in the HTTP response) and the run id (assigned at `run-start`). A continuation body carries the `runId` the client already adopted. The body does **not** carry a `clientId` field — the agent reads the input event off the channel (matched via `inputEventId`) and takes its `clientId` from the publisher's Ably-level `clientId` on the wire message.
...
| -------------- | ---------------- | --------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- | ------------------------------------ |
| `ai-run-start` | Server → Channel | `run-id`, `run-client-id`               | `parent`, `fork-of`, `invocation-id`, `input-client-id`, `input-codec-message-id`, `run-continue`                       | Signal that a run has started        |
| `ai-run-end`   | Server → Channel | `run-id`, `run-client-id`, `run-reason` | `invocation-id`, `input-client-id`, `input-codec-message-id`, `error-code` + `error-message` (when `run-reason: error`) | Signal that a run has ended          |
| `ai-cancel`    | Client → Channel | `run-id`                                | -                                                                                                                       | Request cancellation of a single run |
...
Every domain message - user or assistant - gets a unique `codec-message-id` (a `crypto.randomUUID()`). This is the primary identity for a message throughout the system: the [conversation tree](conversation-tree.md) is indexed by it, the [accumulator](codec-interface.md#accumulator) routes streaming events by it, and [optimistic reconciliation](#optimistic-reconciliation) matches on it.
...
| User message (server relay) | Agent session `Run.addMessages()`               | One UUID per input; if the input already carries an `codec-message-id` header (from the POST body), the existing value is kept |
...
| --------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [Decoder core](decoder.md#message-id-tagging)             | Reads `codec-message-id` from inbound message headers and tags every emitted `DecoderOutput` event with it                                                                                     |
...
| [Conversation tree](conversation-tree.md#data-structures) | Uses the message ID as the primary key (`_nodeIndex`). Branching headers (`parent`, `fork-of`) reference other messages by their message ID                                                    |
...
When a client calls `send()`, it inserts an optimistic message into the conversation tree (with no serial). The server then relays that message onto the channel. When the client receives the relay it matches by `codec-message-id` — via the tree's `codec-message-id → run key` index — and reconciles the optimistic entry with the server-assigned serial ([serial promotion](conversation-tree.md#upsert-the-sole-mutation)) rather than creating a duplicate. The match keys on `codec-message-id`, **not** the `run-id`: an echo reconciles to its optimistic node even when the wire `run-id` differs (or is absent on a fresh send), which keeps reconciliation correct now that `run-id`s are agent-minted.
...
When a user calls `regenerate(messageId)`, the new assistant message carries `fork-of: messageId`. When a user calls `edit(messageId, newMessages)`, the new user message carries `fork-of: messageId`. The [conversation tree](conversation-tree.md#sibling-groups-and-fork-chains) uses these to build sibling groups - alternative responses at the same point in the conversation.
...
| Assistant message       | Agent (`Run.pipe`)            | Priority: explicit `streamOpts.parent` → the most recent user-prompt codec-message-id in `run.view.messages` (populated by the channel-rewind input-event lookup) → `runParent` (= `invocation.parent`). Keeps user → assistant chains explicit without the route having to opt in |
| Continuation amendments | Codec / `Run.pipe`            | Tool outputs and approval responses fold back onto the original assistant message via `codec-message-id` routing - they don't reshape parent edges                                                                                                                                 |
