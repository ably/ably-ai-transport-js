...
| Continuation amendments | Codec reducer                 | Tool outputs and approval responses fold back onto the original assistant message - the reducer attributes them by tool call (`toolCallId`) or by the `codec-message-id` a client resolution addresses. They don't reshape parent edges                                            |
