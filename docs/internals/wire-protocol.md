...
| [Accumulator](codec-interface.md#accumulator)             | Uses `output.messageId` (the codec-message-id) to route decoded events to the correct in-progress domain message (e.g. the `UIMessage` being built). The domain `UIMessage.id` is independent — it is preserved from the source (an assistant id from the stream's `start` chunk `messageId`, a user id from the caller) and is never overwritten with the codec-message-id |
