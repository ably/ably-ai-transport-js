...
| 104011 | `HistoryFetchFailed`         | 500    | A `channel.history()` page fetch failed after retries while paginating history (a `loadOlder` reveal, the agent's `run.view` drain, or a `fetchRawHistory` read). Also rejected by `fetchRawHistory` when its `maxPages` cap is reached with history still remaining                       | Surfaced on the client as the view's `loadError` (and `useView`'s `loadError`); a `fetchRawHistory` call rejects directly. Retry the load and check network connectivity; for the `maxPages` case raise the cap or bound the read with `sinceSerial` |
