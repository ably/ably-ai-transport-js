...
| 104010 | `InputEventNotFound`       | 500    | The agent attached and waited for the input event(s) the invocation points at (rewind + live wait) but `inputEventLookupTimeoutMs` lapsed without seeing them                                           | Check that the client published the input, and that the timeout/buffer limits are adequate |
