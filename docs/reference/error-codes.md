...
| 104010 | `InputEventNotFound`       | 504    | The agent attached and waited for the input event the invocation points at (rewind + live wait) but `inputEventLookupTimeoutMs` lapsed without seeing it                                                | Check that the client published the input, and that the timeout/buffer limits are adequate |
