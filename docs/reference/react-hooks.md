...
| `send(events, options?)`            | `(events: TInput \| TInput[], options?: SendOptions) => Promise<ActiveRun>`                    | Send one input message in this view's branch context. At most one new message (a user message) per send; the array form carries only the wire-only inputs that resolve a single assistant turn (tool results, approval responses). Compose with codec factories (e.g. `codec.createUserMessage(message)`, `codec.createToolResult(...)`); routing fields (`parent`, `target`, `codecMessageId`) live on each input                      |
