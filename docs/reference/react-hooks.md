...
| `historyPageSize` | `number?`                                       | Wire-message limit per `channel.history()` fetch (default `100`); see [Page size](../features/history.md#page-size)                                                                                  |
...
The session is a pure Ably-channel transport — it never sends HTTP. To wake a serverless agent, POST `run.toInvocation().toJSON()` to your endpoint from the `ClientRun` that `view.send`/`regenerate`/`edit` returns. (For the `useChat` integration, use `ChatTransportProvider`, which issues this POST for you.)
...
| ----------------------------------- | -------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `messages`                          | `CodecMessage<TMessage>[]`                                                                               | The visible messages, concatenated across all visible Runs via the codec, each paired with its codec-message-id (`{ codecMessageId, message }`). Read the domain object from `message`; correlate back to the View (`runOf`, `branchSelection`, `regenerate`, `edit`) via `codecMessageId`                                                                                                                                              |
...
| `loadOlder()`                       | `() => Promise<CodecMessage<TMessage>[]>`                                                                | Reveal older messages (uses the hook's `limit`); resolves to the revealed page (oldest-first), `[]` if nothing was revealed. No-op if already loading                                                                                                                                                                                                                                                                                   |
| `runOf(codecMessageId)`             | `(codecMessageId: string) => RunInfo \| undefined`                                                       | Look up the `RunInfo` for the Run that owns `codecMessageId`. `undefined` when the codec-message-id hasn't been observed. Reactive to Run lifecycle — see below                                                                                                                                                                                                                                                                         |
| `run(runId)`                        | `(runId: string) => RunInfo \| undefined`                                                                | Direct lookup of a Run's `RunInfo` by runId. `undefined` when the Run hasn't been observed. Reactive to Run lifecycle — see below                                                                                                                                                                                                                                                                                                       |
| `runs()`                            | `() => RunInfo[]`                                                                                        | Snapshot of the visible Runs along the selected branch, in chronological order. Returns `[]` when the view isn't resolved. Reactive to Run lifecycle — see below                                                                                                                                                                                                                                                                        |
| `branchSelection(codecMessageId)`   | `(codecMessageId: string) => BranchHandle<TMessage>`                                                     | The branch handle anchored at `codecMessageId` (`{ hasSiblings, siblings, index, selected, select }`). Always returns a safe handle — drives navigation arrows, resolves the selected sibling, and switches sibling via `select(index)`. Call `branchSelection(codecMessageId).select(index)` to navigate; `index` is clamped to `[0, siblings.length - 1]`, and `select` is a silent no-op when `codecMessageId` isn't a branch anchor |
| `send(events, options?)`            | `(events: TInput \| TInput[], options?: SendOptions) => Promise<ClientRun<TMessage>>`                    | Send one input message in this view's branch context. At most one new message (a user message) per send; the array form carries only the wire-only inputs that resolve a single assistant turn (tool results, approval responses). Compose with codec factories (e.g. `codec.createUserMessage(message)`, `codec.createToolResult(...)`); routing fields (`parent`, `target`, `codecMessageId`) live on each input                      |
| `regenerate(messageId, options?)`   | `(messageId: string, options?: SendOptions) => Promise<ClientRun<TMessage>>`                             | Fork an assistant message with no new user input                                                                                                                                                                                                                                                                                                                                                                                        |
| `edit(messageId, inputs, options?)` | `(messageId: string, inputs: TInput \| TInput[], options?: SendOptions) => Promise<ClientRun<TMessage>>` | Fork a user message with replacement content                                                                                                                                                                                                                                                                                                                                                                                            |

**Run lifecycle re-renders.** The hook subscribes to the view's `run` event alongside `update`, so a visible Run starting, suspending, resuming, or ending re-renders the component. A status-only transition leaves the message list untouched and so arrives on `run`, not `update` — the subscription is what lets a UI gated on Run status learn that the status moved. Derive streaming state in the render body:

```typescript
const { messages, runOf } = useView();

const latestRun = runOf(messages.at(-1)?.codecMessageId ?? '');
// 'suspended' is paused awaiting input, not streaming — there is no live
// stream to abort, so a Stop button gates on 'active' alone.
const isStreaming = latestRun?.status === 'active';
```

`runOf`, `run`, and `runs` are referentially stable and read through to the view on every call, so it is the re-render that refreshes the answer, not a new callback. Call them during render rather than deriving inside a `useMemo` or `useEffect` keyed on one of them — that memo would not recompute and would hold a stale status. The same goes for `RunInfo.steps`, which advances with output rather than with lifecycle.
...
Branch navigation (`branchSelection().select()`) and write operations (`send()`, `regenerate()`, `edit()`) are on `ViewHandle` from `useView()`, not `TreeHandle`. The tree provides structural queries that are the same regardless of which branch is selected.
