...
| `sendMessage(messages, options?)`       | `(messages: TMessage \| TMessage[], options?: SendOptions) => Promise<ActiveRun>`              | Send user messages in this view's branch context                                                                                                   |
| `sendInput(events, options?)`           | `(events: TInput \| TInput[], options?: SendOptions) => Promise<ActiveRun>`                    | Send input events (tool results, approval responses, regenerate signals); routing fields (`parent`, `target`, `codecMessageId`) live on each input |
