...
The session is a pure Ably-channel transport — it never sends HTTP. To wake a serverless agent, POST `run.toInvocation().toJSON()` to your endpoint from the value `view.send`/`regenerate`/`edit` returns. (For the `useChat` integration, use `ChatTransportProvider`, which issues this POST for you.)
...
| `getMessagesWithIds()`                  | `() => CodecMessage<TMessage>[]`                                                               | The visible messages each paired with its codec-message-id (`{ codecMessageId, message }`) — use to correlate a rendered message back to the transport (the domain `message.id` is not the codec-message-id)                                                                                           |
...
**Returns:** `ViewHandle<TInput, TMessage>` - the same handle type as `useView()`, with nodes, pagination, navigation, and write operations. Returns an empty handle (no nodes, no messages) when no session is provided or `skip` is `true`.
