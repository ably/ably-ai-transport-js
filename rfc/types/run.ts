...
export interface ClientRun<TMessage> extends Run<TMessage> {
   ...
   * Vercel codec, `UIMessage.id` is required and is reused as the
   * transport-level `x-ably-msg-id`; no ID is generated by the SDK. The
   * message is published tagged with this run's ID. Used for the initial
   * user message (after start) and for mid-run steering. Does not require
   * an agent to be running — the message is durable.
   */
