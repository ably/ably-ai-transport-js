 ...
 * already knows) and/or by `input-transport-message-id` (a fresh send, whose run-id
 * the agent mints at run-start — before then the client can only key the cancel
 * by the triggering input's transport-message-id). At least one is present.
 *
...
import { EVENT_CANCEL, HEADER_EVENT_ID, HEADER_INPUT_TRANSPORT_MESSAGE_ID, HEADER_RUN_ID } from '../../constants.js';
import { getTransportHeaders } from '../../utils.js';

/**
 * The identifier(s) a cancel signal targets its run by. At least one of
 * `runId` / `inputTransportMessageId` must be set for the cancel to route.
 */
export interface CancelTarget {
  /** The run-id to cancel — set for continuations, whose run-id the caller knows. */
  runId?: string;
  /**
   * The triggering input's transport-message-id to cancel — set for fresh sends,
...
export const readCancelTarget = (msg: Ably.InboundMessage): CancelTarget => {
  ...
  return {
    ...
    inputTransportMessageId: headers[HEADER_INPUT_TRANSPORT_MESSAGE_ID],
  };
};
