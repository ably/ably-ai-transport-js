...
class DefaultClientSession<TEvent, TProjection, TMessage> implements ClientSession<TEvent, TProjection, TMessage> {
  ...
  constructor(options: ClientSessionOptions<TEvent, TProjection, TMessage>) {
    ...
    // Seed tree with initial messages — session assigns its own runId / msgId
  ...
  private _handleChannelStateChange(stateChange: Ably.ChannelStateChange): void {
    ...
    // _ownRunIds here — late events must still accumulate into the tree.
    // The run-end handler cleans up local maps.
    //
  ...
  private _closeMatchingRunStreams(filter: CancelFilter): void {
    // Only close the router streams here. The Tree's RunNode + projection
    // stays alive so that late agent events (e.g. abort,
    // x-ably-status: aborted) arriving before run-end still fold into the
    // Run's projection. The run-end handler clears active tracking.
  ...
  private async _internalSend(
    ...
    // msg-id of its tail message before calling this delegate, so neither
    ...
    // provided. The View pre-resolves the msg-id of the last visible message
    ...
    for (const item of classified) {
      if (item.kind === 'regenerate') {
        ...
        // its promptId and reads parent/regenerates from these headers,
