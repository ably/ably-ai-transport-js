/**
 * Core server-side transport types, parameterized by codec event and message types.
 *
 * These types define the contract for the server transport implementation,
 * independent of which codec (Vercel AI SDK, etc.) is used.
 */
...
export interface NewTurnOptions<TEvent> {
   ...
   * publish failure, encoder recovery failure, pipe stream errors.
   */
  onError?: (error: Ably.ErrorInfo) => void;
}
