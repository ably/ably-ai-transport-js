...
export type RunLifecycleEvent =
  | {
      ...
      parent?: string;
      /**
       * The msg-id of the user prompt being forked, when the run is an
       * edit. Carried verbatim from the `x-ably-fork-of` wire header.
       */
      forkOf?: string;
      /**
       * The msg-id of the assistant message this run regenerates, when
...
export interface RunNode<TProjection> {
   ...
   * observed message's `x-ably-parent` header via the msgId -> runId index.
   ...
   * a msg-id that has been observed; the Tree resolves it to a runId via
   * the msgId -> runId index.
   */
  forkOf: string | undefined;
  /**
   * The msg-id this Run regenerates, or `undefined` for non-regenerate
   ...
   * named by `regeneratesMsgId` is replaced by this Run's content when
   * the View materialises the chain into messages (Spec: AIT-CT13d).
   */
  regeneratesMsgId: string | undefined;
  /**
...
export interface Tree<TProjection> {
  ...
  /**
   * Get the Run that owns a given msg-id (via the Tree's msgId -> runId index),
   * or undefined if the msg-id hasn't been observed.
   */
   ...
   * - **Regenerate groups** — the Run that owns a regenerated msg-id +
   *   every Run whose `regeneratesMsgId` points at that msg-id.
   *
   ...
   *
   * The group is anchored at a msg-id — the one being regenerated — and
   * its members are the Run that owns that msg-id plus every Run whose
   * `regeneratesMsgId` points at it. Returns `undefined` when `runId`
   * neither regenerates a known msg-id nor owns a msg-id that has been
   * regenerated.
   * @param runId - The runId to look up.
   * @returns Anchor msg-id and members ordered chronologically by
   *   startSerial (owner first), or `undefined` if there is no group.
   */
  getRegenerateGroup(runId: string):
    | {
        /** The msg-id this group regenerates. */
        anchorMsgId: string;
...
export interface View<TEvent, TProjection, TMessage> {
  ...
  /**
   * Whether the message at `msgId` is a branch-point anchor — i.e. the UI
   * should render navigation arrows next to this specific bubble.
   *
   * Per AITRFC-014, branch points are msg-anchored: edit forks point at a
   * prompt-id (the user prompt), regenerate forks point at a msg-id (the
   * assistant message). This is finer-grained than the Run-keyed
   * `hasSiblingRuns(runId)`: a Run that owns multiple messages will be
   * "in a sibling group" via its runId, but only the message that
   ...
   * @param msgId - The msg-id of the bubble being rendered.
   * @returns True iff `msgId` is the branch anchor of a sibling group.
   */
  hasMessageSiblings(msgId: string): boolean;

  /**
   * Get the sibling Runs forming the branch point anchored at `msgId`.
   * Returns the same Runs as {@link getSiblingRuns} would for the owning
   * Run's group, but only when `msgId` is the actual branch anchor; for
   * non-anchor msg-ids the array is empty even if the owning Run has
   * siblings.
   * @param msgId - The msg-id of the bubble being rendered.
   * @returns Ordered sibling Runs, or `[]` if `msgId` is not a branch anchor.
   */
  getMessageSiblings(msgId: string): RunNode<TProjection>[];

  /**
   * Index of the currently selected sibling Run for the branch point
   * anchored at `msgId`. Returns `0` if `msgId` is not a branch anchor.
   * @param msgId - The msg-id of the bubble being rendered.
   */
  getSelectedMessageSiblingIndex(msgId: string): number;

  /**
   * Select a sibling Run at the branch point anchored at `msgId`. Updates
   * this view's branch selection and emits `update`. No-op when `msgId`
   * is not a branch anchor or `index` is out of range.
   * @param msgId - The msg-id of the bubble being rendered.
  ...
  /**
   * Get the Run that owns a given msg-id, or undefined if the msg-id hasn't
   * been observed. Useful when the UI holds a msg-id (e.g. from a previous
   * render) and needs the owning Run.
   */
  getRunByMsgId(msgId: string): RunNode<TProjection> | undefined;
