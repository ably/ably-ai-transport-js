...
// public surface as `ActiveRun` and `ClientSession`.
...
export interface ClientRun<TInput extends CodecInputEvent, TMessage> extends BaseRun<TMessage> {
   ...
   * carries this Run's `run-id` header so the agent's Tree folds it into
   * the active Run's projection. Pass the same shape `view.send(...)`
   * accepts: typically `codec.createUserMessage(...)` for a follow-up user
   * message, but any `TInput` the codec defines is permitted.
   *
   ...
   * `outcome` for the consumed/not-consumed determination derived from
   * the union of `steer-codec-message-ids` stamps the Run's responses
   * carry.
   *
   * The SDK awaits {@link runId} internally if it has not yet resolved,
   * so this call is safe to make as soon as the handle is returned by
   * {@link ClientSession.view}'s send / regenerate / edit. If the run-id
   * never resolves (e.g. invocation failed), both returned promises
   * reject.
   *
   * Once the SDK has folded a `run-end` for this Run, the handle is dead
   * and subsequent `steer()` calls return immediately-rejected promises
   * — no channel publish is attempted. The application recovers by
   * issuing a new `view.send(...)` + invocation.
   * @param input - The codec input event to publish, in the codec's input shape.
   * @returns Two promises: `published` (publish acknowledgement) and
   *   `outcome` (consumed/not-consumed at the next terminal event).
   */
