...
// TODO reword this comment to reflect the fact that we are intentionally stripping some of the duplicate data from these event types
// so that we don't reproduce/duplicate that data on the wire. e.g. every "done" ending bracket item contains the full content that
// was streamed as deltas since it's associated/matching opening bracket.
/**
 * Reduce a completed output item to its {@link WireDoneItem} wire shape.
...
export const outputs = ({
  ...
  // Three groups share the content_part.added start, told apart by the added
  ...
  //
  // TODO: it's not obvious that logprods do ride here!?
  ...
  // (its `part.logprobs` comes through empty, unexpected as that is), so it
  ...
  // item envelopes above, the item rides under the `item` key of its wire
