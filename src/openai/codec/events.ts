 ...
 * - the five streamed families' `*.done`/`.delta` closes ({@link
 *   import('./descriptors.js').outputs}' `decodeEnd`/`decodeDelta` escape
 ...
 *   family's stream without having seen the real owner envelope; both events'
...
/**
 * A completed message's content part slimmed to the one datum the reducer can't
 ...
 * with `bytes`) and a refusal part slims to just its `type`. Carried
 * index-aligned with the message's `content` so the reducer folds each part
 * into its slot.
 */
export type SlimDoneContentPart = PickPresent<Responses.ResponseOutputMessage['content'][number], 'type' | 'logprobs'>;

/**
 * The wire-slim shape of a completed output item. A real `response.output_item.done`
 ...
 * by the streamed deltas — see {@link SlimDoneContentPart}, present only when
 ...
 * documents in full why logprobs ride the finalised item (and the OpenAI
 * accumulator that choice mirrors).
 */
export type SlimDoneItem =
  | (Pick<Responses.ResponseOutputMessage, 'id' | 'type' | 'status'> & {
      /**
       * The message's content parts slimmed to their per-part `logprobs`
 ...
 * slimmed to {@link SlimDoneItem} (the only shapes the codec ever emits — see
 * `slimDoneItem`), or an {@link UnmodelledOutputItem} at its full, real shape.
 *
 * The unmodelled half exists only for type assignability, not because such
 * items are ever published: `slimDoneItem` throws on an unmodelled type before
 ...
 * to hold. Leaving them full rather than slimming them is what keeps that
 * assignment legal — some hosted-tool item types carry `status` literals (e.g.
 * `'searching'`) no modelled variant's `status` allows.
 */
export type DoneItem = SlimDoneItem | UnmodelledOutputItem;
 ...
 * (any item type) is still assignable, since `DoneItem` only slims the three
 * item types the decoder ever actually reconstructs and leaves every other
 * type at its real shape. `slimDoneItem` (`descriptors.ts`) accepts this same
 * type on encode for exactly this reason.
 */
type WithSlimDoneItem<T> = T extends { type: 'response.output_item.done' } ? Omit<T, 'item'> & { item: DoneItem } : T;
 ...
 * from the finalised item (see {@link SlimDoneItem}), never this streamed close,
 * so the reconstruction genuinely has none and the reducer never reads them
 * here. Stripping the field keeps the `decodeEnd` reconstruction honest — it
 ...
 * slimmings the codec applies, composed: `sequence_number` stripped from the
 * reconstructed events ({@link WithoutSequenceNumber}), `output_item.done`'s
 * `item` slimmed ({@link WithSlimDoneItem}), and `output_text.done`'s `logprobs`
 ...
 */
type SlimResponseStreamEvent = WithoutTextDoneLogprobs<
  WithSlimDoneItem<WithoutSequenceNumber<Responses.ResponseStreamEvent>>
>;

/**
 * `TOutput` — a {@link SlimResponseStreamEvent} (the Responses stream events as
 ...
 * at compile time.
 */
export type OpenAIOutput = SlimResponseStreamEvent | FunctionCallOutputEvent;
 ...
 * if a future change to `WithoutSequenceNumber` / `WithSlimDoneItem` /
 ...
 * through (today, only `controller.enqueue(value)` in the demo's
 * `agent-stream.ts`, which isn't even part of this package's own `tsc` run).
 */
...
/**
 * A single item within a turn — every shape a stored turn can hold. All are
 * members of `ResponseInputItem`, so a turn is valid `/responses` input as-is,
 * with no conversion (see `toResponsesInput`). An assistant turn holds
 * {@link ModelledOutputItem}s folded from the stream, plus the function-call
 * output items the codec authors for server-run tools; a user turn holds a
