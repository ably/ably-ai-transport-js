...
export interface UseHistoryOptions {
  /**
   * How many messages to read per page. A walk opens with the value current
   * when it opens and keeps it for its whole life, since the later pages come
   * from the page's own cursor and carry the size it was opened with. A change
   * leaves the open walk and its items alone and applies to the next walk,
   * which starts when the provider recreates the transport.
   */
  limit: number;
...
export const useHistory = <E = unknown>(options: UseHistoryOptions): HistoryHandle<E> => {
  ...
  // The page size to open a walk with. Read only when a walk opens, so a
  // caller that changes `limit` reaches the next walk and leaves the open one
  // as it is. Keeping it out of `loadMore`'s dependencies is what holds the
  // walk together: a change there would rebuild the callback, and the effect
  // below would reset the list and reopen the walk from the attach point.
  ...
  const loadMore = useCallback(async (): Promise<void> => {
    ...
    try {
      const page = last === undefined ? await transport.history({ limit: limitRef.current }) : await last.next();
      ...
      // Only the walk that still owns the transport clears the flag. A page
      // from a superseded walk reaching here would report idle while the new
      // walk's first page is in flight, and a `loadMore` call then would read
      // a second page under the same cursor.
      if (currentRef.current === transport) loadingRef.current = false;
    }
  }, [transport]);
