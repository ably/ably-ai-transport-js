...
import { DOMAIN_HEADER_PREFIX, HEADER_ROLE, HEADER_TURN_ID } from '../../constants.js';
 ...
 * rather than a streaming lifecycle event. Discrete message parts are distinguished
 * by the presence of the `x-domain-messageId` header, which `encodeMessagePayloads`
 * always sets. Streaming events published via `appendEvent` never carry this header.
 * @param name - The Ably message name to check.
 * @param headers - The Ably message headers to inspect for messageId presence.
...
const isDiscreteMessagePart = (name: string, headers: Record<string, string>): boolean =>
  ...
  `${DOMAIN_HEADER_PREFIX}messageId` in headers;
...
const decodeDiscretePayload = (input: MessagePayload, lifecycle: LifecycleTracker<AI.UIMessageChunk>): Out[] => {
  ...
  // Distinguished from lifecycle events by the presence of x-domain-messageId.
