...
export interface VercelRegenerateInput {
  /** Discriminator. */
  kind: 'regenerate';
  /** Where the conversation now ends, so the agent knows what to answer from. */
  payload: {
    /**
     * The `UIMessage.id` of the last message the conversation still holds
     * after useChat truncated it, so the agent answers from this point on.
     * Typically the user turn whose reply is being redone, because useChat
     * drops the assistant message it is regenerating before it asks.
     *
     * The transport carries the field and never reads it; what the agent does
     * with it — typically truncating its stored conversation after this
     * message — is the application's choice.
     */
    messageId: string;
  };
}
