/**
 * The session codec's Vercel input (`ai-input`) descriptors — the wire mapping
 * for the session layer's five-variant input taxonomy, the user-message
 * fan-out included. The public wire codec's input table lives in `inputs.ts`.
 *
...
import type { ForkSeed, VercelSessionInput } from './session-events.js';
import {
  asString,
 ...
 * @param builder - The `{ event, batch }` builder curried on `VercelSessionInput`.
...
export const sessionInputs = ({
  event,
  batch,
}: InputBuilder<VercelSessionInput>): readonly InputDescriptor<VercelSessionInput>[] => [
