 ...
 *
 * On the **hydration path** {@link ChatTransport.readSince} reads channel
 ...
 * The history read also holds the events of any run that has not ended so
 ...
 *   input anchored on the last message useChat's truncated list still holds.
 ...
 * construction so nothing published before the hydration read is lost, and an
...
/**
 * One history event, tagged with the direction it travelled.
 *
...
> {
   ...
   */
  events: HistoryEvent<TMetadata, TDataParts, TTools>[];
}

/**
 * What {@link ChatTransport.readSince} reads off the channel.
 * @template TMetadata - Per-message metadata type.
 * @template TDataParts - Custom data-part types.
 * @template TTools - Tool set typing tool parts.
 */
...
> {
  /**
   * The events read from history, grouped by the message they belong to, oldest first.
   ...
   * one producer.
   */
  messages: HistoryMessage<TMetadata, TDataParts, TTools>[];
  /** True when the history read reached the channel start, so there is no older history to page. */
  exhausted: boolean;
}
 ...
 * the hydration read, a cancel the SDK interface leaves out, and the
...
> extends SdkChatTransport<AI.UIMessage<TMetadata, TDataParts, TTools>> {
  /**
   * Read the channel backwards from the attach point down to `latestSerial`
   ...
   * {@link HistoryMessage} for why the reducer takes one call per group.
   *
   * Subscribes before it pages, so nothing published during the history read is lost.
   ...
   * hint and no run seen open since construction, reconnecting before the history read
   * has run returns `null`.
   * @param latestSerial - The channel serial of the newest message the application's store holds. Every message at or before it must be complete in the store. Omit to read to the channel start.
   * @returns The message groups and whether the history read reached the channel start.
   */
  ...
  /**
   * Subscribe to runs this client is not streaming. useChat accepts new
   * streamed content only through `resumeStream()`, so an application on a
   * shared channel calls that in response to observe another participant's
   * run.
   *
   * Fires only while this adapter is idle, so it cannot fight the client's own
   * send: on a run starting live while idle, and again on the adapter going
   * idle while a run it saw start is still outstanding. That second
   * announcement is what reaches a run another participant started while this
   * client was busy streaming, which the first would have passed over.
   *
   * The same run is announced again on each idle transition until it ends, so
   * an application that declines one gets another chance rather than losing
   * the run.
   * @param callback - Called with the run's id to observe it.
   * @returns An unsubscribe function.
   */
 ...
 * seen. A second read can put one event in both the retention and the live
...
> implements ChatTransport<TMetadata, TDataParts, TTools> {
   ...
   * exactly once and the history read shares it — an event dropped before a collector
   ...
   * not ended. Insertion order carries meaning in two places: the newest key
   * is the run {@link reconnectToStream} resumes, and the oldest is what the
   * retention cap evicts.
   *
   * A run's entry grows as its live events arrive — its terminal, and whatever
   * a superseded stream delivered — and a later read merges its pages in
   * rather than replacing what is held. Only {@link reconnectToStream}
   * delivering the message releases an entry, or the cap evicting it.
   */
  private readonly _retained = new Map<string, AdapterEvent<TMetadata, TDataParts, TTools>[]>();
  /**
   * Every page {@link readSince} has read so far, oldest first. The
   * underlying pager opens its cursor once and keeps it, so a second read
   * fetches no pages — without this the second read would see an empty channel,
   * report no messages and drop the retention the first read built.
   */
  private _historyPages: AdapterEvent<TMetadata, TDataParts, TTools>[] = [];
  /** Whether the history read has reached the start of the channel. */
  private _historyExhausted = false;
  /**
   * Runs seen open on the channel — a `run-lifecycle` start with no end yet,
   * newest last. This is what lets a reconnect go live on a run the history read never
  ...
  async sendMessages(
    ...
    if (options.trigger === 'regenerate-message') {
      return this._sendRegenerate(options.messages, options.abortSignal);
    }
  ...
  async readSince(latestSerial?: string): Promise<ReadSinceResult<TMetadata, TDataParts, TTools>> {
    if (this._closed) {
      throw new Ably.ErrorInfo('unable to read history; the chat transport is closed', ErrorCode.SessionClosed, 400);
    }
    ...
    // are strictly newer than every page; a second read's overlap is removed by
    // delivery at hand-off.

    let all: AdapterEvent<TMetadata, TDataParts, TTools>[] = [...this._historyPages];
    let exhausted = this._historyExhausted;
    while (!exhausted) {
    ...
    this._historyPages = all;
    this._historyExhausted = exhausted;
    ...
    const openRuns = this._openRunsIn(historyEvents);
    this._retain(historyEvents, openRuns);

    const messages = this._groupHistory(historyEvents, openRuns);
    this._logger.debug('ChatTransport.readSince(); read complete', {
  ...
  async reconnectToStream(
    ...
    // the history read withheld a message for, then the newest run seen starting live
    ...
    // the events the buffer holds. A second read can put one event in both, so the
    ...
    // in behind it.
    //
    // The retained terminal goes last. A run that ended between the history read and
   ...
   * @param messages - useChat's message list, already truncated for the regenerate.
   * @param abortSignal - useChat's per-send abort signal.
   * @returns The new run's chunk stream.
   */
  private async _sendRegenerate(
    messages: readonly AI.UIMessage<TMetadata, TDataParts, TTools>[],
    abortSignal: AbortSignal | undefined,
  ): Promise<ReadableStream<AI.UIMessageChunk>> {
    // useChat truncates its list before it asks, so the last message left is
    // the point the conversation now ends at. That anchor is what the agent
    // needs, and reading it from the list works whether or not the caller
    // named a message: `regenerate()` with no argument drops the trailing
    // assistant message and leaves the user turn that prompted it.
    const anchor = messages.at(-1);
    if (!anchor) {
      throw new Ably.ErrorInfo('unable to regenerate; the message list is empty', ErrorCode.InvalidArgument, 400);
    }
    const transportMessageId = this._claimInputId();
    return this._send(abortSignal, async () =>
      this._transport.publishInput({ kind: 'regenerate', payload: { messageId: anchor.id } }, { transportMessageId }),
    );
  }
  ...
  /**
   * Whether the history read has reached back past the store's serial.
  ...
  /**
   * The runs in the history read that published no `ai-run-end`. Their messages are
   * the adapter's to deliver, not the store's.
   * @param historyEvents - The events read from history, oldest first.
   * @returns The open runs' ids.
   */
  private _openRunsIn(historyEvents: AdapterEvent<TMetadata, TDataParts, TTools>[]): Set<string> {
    const seen = new Set<string>();
    ...
    for (const event of historyEvents) {
      if (event.kind === 'run-lifecycle') {
        ...
        // a suspend means the agent stopped, and history carries the whole
        // message.
  ...
  /**
   * File each open run's events for a later replay, newest run last.
   *
   * Nothing here removes an entry. A retention exists because this adapter
   * took on delivering that run's message. It is released when
   * {@link reconnectToStream} delivers the message, or when the cap evicts it.
   *
   * History cannot release one. Its pages end at the attach point and only
   * reach further back, and a run's end is newer than its start, so a page set
   * holding the start holds the end too. An end published after attach arrives
   * live and reaches no page at all. So every read reports a given run the same
   * way, and a run this read does not mention still has a withheld message:
   * its end came live, or the run began after attach.
   * @param historyEvents - The events read from history, oldest first.
   * @param openRuns - The runs whose messages are withheld.
   */
  private _retain(historyEvents: AdapterEvent<TMetadata, TDataParts, TTools>[], openRuns: Set<string>): void {
  ...
  /**
   * Group the events read from history by the message they belong to, skipping the
   * withheld runs.
   *
   ...
   * that names one this read never saw becomes its own group, so nothing
   * published is dropped.
   * @param historyEvents - The events read from history, oldest first.
   * @param openRuns - The runs whose messages are withheld.
   * @returns The groups, oldest first.
   */
  private _groupHistory(
    historyEvents: AdapterEvent<TMetadata, TDataParts, TTools>[],
    openRuns: Set<string>,
  ): HistoryMessage<TMetadata, TDataParts, TTools>[] {
    const order: string[] = [];
    const buckets = new Map<string, HistoryEvent<TMetadata, TDataParts, TTools>[]>();
    // The domain message id each wire bucket turned out to belong to.
    const domainOf = new Map<string, string>();

    const push = (key: string, entry: HistoryEvent<TMetadata, TDataParts, TTools>): void => {
    ...
    for (const event of dropSupersededAttempts(historyEvents)) {
      if (event.kind !== 'message') continue;
    ...
    const groups: HistoryMessage<TMetadata, TDataParts, TTools>[] = [];
    const byId = new Map<string, HistoryMessage<TMetadata, TDataParts, TTools>>();
  ...
  /**
   * Record a run's history events as its retention, keeping whatever live
   * events it already holds.
   *
   * A second call sees the same pages as the first, so it derives the run's
   * history again and nothing newer. Replacing the retention outright would
   * throw away the terminal {@link _trackOpenRun} appended and the events a
   * live supersede handed over, leaving a resume that replays only history and
   * has no terminal to close its stream with.
   *
   * The history events go first, because paging may have reached further back
   * than the retention does, and the live events are newer than every page.
   * @param runId - The run the events belong to.
   * @param historyEvents - The run's events read from history, oldest first.
   */
  private _retainHistory(runId: string, historyEvents: AdapterEvent<TMetadata, TDataParts, TTools>[]): void {
    const held = this._retained.get(runId) ?? [];
    this._recordRetention(runId, dedupeByDelivery([...historyEvents, ...held]));
  }

  /**
   * Append live events to a run's retention, so a consumer repairing a stream
   * has the whole run to replay rather than only its history.
   * @param runId - The run the events belong to.
   * @param events - The live events, oldest first.
   */
  private _retainLive(runId: string, events: AdapterEvent<TMetadata, TDataParts, TTools>[]): void {
    const held = this._retained.get(runId) ?? [];
    this._recordRetention(runId, dedupeByDelivery([...held, ...events]));
  }

  /**
   * Write a run's retention, re-keying it as newest and evicting past the
   * bound.
   *
   * Insertion order carries meaning: the newest key is the run
   * {@link reconnectToStream} resumes, and the oldest is what the cap evicts.
   * Deleting the key before setting it is what moves an existing entry to the
   * end of that order.
   * @param runId - The run the events belong to.
   * @param events - Its events, oldest first.
   */
  private _recordRetention(runId: string, events: AdapterEvent<TMetadata, TDataParts, TTools>[]): void {
    this._retained.delete(runId);
   ...
   * and {@link _announceOutstanding} reads the same set to offer that run to
   * the application unprompted.
   * @param event - The classified transport event.
   */
  private _trackOpenRun(event: AdapterEvent<TMetadata, TDataParts, TTools>): void {
    ...
    if (type === 'end' || type === 'suspend') {
      ...
      // run that ends between the history read and the reconnect still has a withheld
    ...
    // Delete before adding, so the id moves to the end and the set's
    // iteration order stays newest-last.
    this._unendedRunIds.delete(runId);
  ...
  private _notifyForeignRun(event: AdapterEvent<TMetadata, TDataParts, TTools>): void {
    ...
    this._emitForeignRun(runId);
  }

  /**
   * Tell the foreign-run subscribers about a run still outstanding now that
   * this adapter has gone idle.
   *
   * {@link _notifyForeignRun} speaks only while idle, so a run another
   * participant started while this client was streaming was never announced.
   * A resume that replays an already-ended run reaches idle again straight
   * away, and without this second announcement that live run stays invisible
   * until the application hydrates afresh.
   *
   * Deferred to a task of its own. A subscriber answers by calling useChat's
   * `resumeStream`, and the replay that closes a stream reaches here from
   * inside `reconnectToStream`, before useChat holds the stream it asked for —
   * a resume raised there would race the one already in flight.
   *
   * A subscriber that declines hears about the run again on the next idle
   * transition, which only comes around when a stream closes.
   */
  private _announceOutstanding(): void {
    if (this._foreign.size === 0) return;
    setTimeout(() => {
      if (this._closed || this.streaming) return;
      const runId = [...this._unendedRunIds].findLast((id) => !this._streamingRunIds.has(id));
      if (runId === undefined) return;
      this._logger.debug('ChatTransport._announceOutstanding(); announcing an outstanding run', { runId });
      this._emitForeignRun(runId);
    }, 0);
  }

  /**
   * Hand a run id to every foreign-run subscriber, isolating each callback so
   * one that throws does not stop the rest.
   * @param runId - The run to announce.
   */
  private _emitForeignRun(runId: string): void {
    for (const callback of this._foreign) {
      try {
        callback(runId);
      } catch (error) {
        this._logger.error('ChatTransport._emitForeignRun(); callback threw', { error });
      }
    }
  }
   ...
   *
   * An id already held is deleted before it is added again, so it moves to the
   * end and the set's iteration order stays newest-last, which is what the cap
   * evicts from.
  ...
  private _openRunStream(abortSignal?: AbortSignal): RunCollector<TMetadata, TDataParts, TTools> {
    ...
    const deliver = (event: AdapterEvent<TMetadata, TDataParts, TTools>, id: string): void => {
      ...
      if (event.kind === 'step-lifecycle') {
        ...
        this._retainLive(id, [...delivered, event]);
        finish();
        controller?.error(
          new Ably.ErrorInfo(
