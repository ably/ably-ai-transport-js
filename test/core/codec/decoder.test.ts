...
describe('createDecoderCore', () => {
  ...
  describe('stream end', () => {
    ...
    it('remembers a closer for a serial it does not hold', () => {
      const { logger, debug } = createMockLogger();
      const core = createDecoderCore({ logger });
      const closer = message({ serial: 's2', ai: { ends: 's1' } });
      expect(core.prepare(closer)).toBe(closer);
      expect(debug).toHaveBeenCalledWith(expect.stringContaining('closer before its stream'), { serial: 's1' });
    });

    it('hands the stream of a closer it heard first on whole, without tracking it', () => {
      // History walks newest page first, so a closer on one page is decoded
      // before the stream's message on the older page.
      const { logger, debug } = createMockLogger();
      const core = createDecoderCore({ logger });
      core.prepare(message({ serial: 's2', ai: { ends: 's1' } }));
      const stream = opener({ serial: 's1', data: 'Hello' });
      expect(core.prepare(stream)).toBe(stream);
      expect(debug).toHaveBeenCalledWith(expect.stringContaining('stream already ended'), { serial: 's1' });
      // Not tracked: an update for s1 is first contact and carries its full
      // content rather than a tail.
      const update = message({ serial: 's1', action: 'message.update', data: 'Hello world', version: 's1:v2' });
      expect(core.prepare(update)).toBe(update);
    });

    it('treats a stream met through an append or an update after its closer the same way', () => {
      for (const action of ['message.append', 'message.update'] as const) {
        const core = createDecoderCore();
        core.prepare(message({ serial: 's2', ai: { ends: 's1' } }));
        const first = message({ serial: 's1', action, data: 'x', version: 's1:v2' });
        expect(core.prepare(first)).toBe(first);
        // Not tracked: a replay of the same version is not a dropped duplicate.
        const replay = message({ serial: 's1', action, data: 'x', version: 's1:v2' });
        expect(core.prepare(replay)).toBe(replay);
      }
    });

    it('uses a remembered closer once', () => {
      const core = createDecoderCore();
      core.prepare(message({ serial: 's2', ai: { ends: 's1' } }));
      core.prepare(opener({ serial: 's1', data: 'Hello' }));
      // The record was consumed by the first opener; a second one is first
      // contact again, so it is tracked and a repeat of it is dropped.
      const again = opener({ serial: 's1', data: 'Hello' });
      expect(core.prepare(again)).toBe(again);
      expect(core.prepare(opener({ serial: 's1', data: 'Hello' }))).toBeUndefined();
    });
