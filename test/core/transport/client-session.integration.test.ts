 ...
 * output (stamped with the step-id / attempt-id), then `ai-step-end`. Used to
 ...
 * @param opts.attemptId - This attempt's id.
...
const publishStepAttempt = async (
  ...
  await channel.publish({
    ...
    extras: {
      ai: { transport: buildStepHeaders({ runId: opts.runId, stepId: opts.stepId, attemptId: opts.attemptId }) },
    },
  });
