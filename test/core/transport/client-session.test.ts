...
describe('ClientSession', () => {
  ...
  describe('send', () => {
    it('returns a ClientRun with a synchronous inputCodecMessageId, an empty runId until started, and cancel', async () => {
      const run = await fix.session.view.send({ kind: 'user-message', text: 'hi' });
      // The synchronous routing key (the triggering input's codec-message-id)
      // is known immediately. The agent mints the run-id, so `runId` is empty
      // until `started` resolves; `started` is the latch to await.
    ...
    it('exposes this run’s whole turn via messages, plus status and started → runId', async () => {
      const run = await fix.session.view.send({ kind: 'user-message', text: 'hi' });
      // Before run-start: the optimistic user input is this run's turn-opening
      // message; the run is active with no error and no run-id yet.
      expect(run.messages.map((m) => m.content)).toEqual(['hi']);
      expect(run.status).toBe('active');
      expect(run.error).toBeUndefined();

      // After the agent's run-start: `started` resolves and `runId` is readable;
      // `messages` is still this run's own turn (no output has streamed yet).
  ...
  describe('send — continuation', () => {
    ...
    it('surfaces the continuation trigger event id and run identity on the ClientRun', async () => {
      await fix.session.view.send({ kind: 'user-message', text: 'hi' });
      const { runId } = await ackPendingSend(fix.channel, fix.codec);
    ...
    it('stamps the continuation event-id on the publish and surfaces it on the ClientRun', async () => {
      await fix.session.view.send({ kind: 'user-message', text: 'hi' });
      const { runId } = await ackPendingSend(fix.channel, fix.codec);
      const before = fix.codec.lastEncoder()?.publishCalls.length ?? 0;
  ...
  describe('runId resolution', () => {
    ...
    it('fresh send: run.runId resolves by the triggering input codec-message-id, with the agent-minted run-id', async () => {
      ...
      expect(run.runId).toBe('agent-minted-run-id');
    });
  ...
  // -------------------------------------------------------------------------
  // run.steer — publish a user message into a live Run; outcome derived by
  // membership of the steer's codec-message-id in the union of
  // `steer-codec-message-ids` stamps observed on the Run's response messages.
  // -------------------------------------------------------------------------

  describe('run.steer', () => {
    // Steer, wait for its publish to land on the mock encoder, and return the
    // steer's codec-message-id (read off the publish headers). Tests use it to
    // simulate the agent stamping that id on a subsequent response. Closes over
    // `fix` from the outer describe.
    const steerAndAwaitPublish = async (
      ...
      const publishesBefore = fix.codec.lastEncoder()?.publishCalls.length ?? 0;
      const { published, outcome } = run.steer(fix.codec.createUserMessage({ id, content }));
      let steerCodecMessageId: string | undefined;
      for (let i = 0; i < 100; i++) {
        const calls = fix.codec.lastEncoder()?.publishCalls ?? [];
        if (calls.length > publishesBefore) {
          const headers = calls.at(-1)?.opts?.extras?.headers;
    ...
    it('outcome resolves not-consumed when run-end arrives before the steer echo', async () => {
      ...
      // still settle the outcome — the drain on run-end sweeps the pending
      // echoes for the run, or the outcome promise would orphan indefinitely.
      const run = await fix.session.view.send({ kind: 'user-message', text: 'hi' });
      const { runId } = await ackPendingSend(fix.channel, fix.codec);
      await run.started;
    ...
    it('accumulates the union across multiple response stamps for the run', async () => {
      ...
      // Two responses, each stamping one steer id (per-step-attempt delta).
      simulateMessage(
        fix.channel,
        ablyMsg('ai-output', {
          [HEADER_RUN_ID]: runId,
  ...
  describe('regenerate events', () => {
    ...
    it('mints a fresh event-id for the regenerate input and surfaces it on the ClientRun', async () => {
