...
describe('ClientTransport', () => {
  ...
  describe('message routing', () => {
    ...
    it('multi-message send chains messages so editing the first hides the second', async () => {
      ...
      let ids = transport.view.flattenNodes().map((n) => n.message).map((m) => m.id);
      expect(ids).toContain('u1');
      expect(ids).toContain('u2');
      ...
      ids = transport.view.flattenNodes().map((n) => n.message).map((m) => m.id);
  ...
  describe('channel continuity loss', () => {
    it('detects discontinuity on pre-attached channel without needing initial attach event', async () => {
      ...
      const turn = await preAttachedTransport.send({ id: 'u1', content: 'hi' });
      await mockFetch.waitForCalls(1);
    ...
    // After going SUSPENDED → ATTACHING → ATTACHED with resumed: false
    it('errors active streams when re-attaching with resumed: false', async () => {
      ...
      // The SUSPENDED transition already errors the stream, but simulate
      // the full realistic sequence to verify re-attach is also detected
      simulateStateChange(channel, {
        current: 'suspended',
        previous: 'attached',
        resumed: false,
    ...
    it('errors multiple active streams on continuity loss', async () => {
      ...
      const turn1 = await transport.send({ id: 'u1', content: 'hi' });
      const turn2 = await transport.send({ id: 'u2', content: 'hey' });
      await mockFetch.waitForCalls(2);
