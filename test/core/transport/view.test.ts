...
const makeTestCodec = (): Codec<TestEvent, TestProjection, TestMessage> => ({
  ...
  fold: (state, event, meta) => {
    if (event.type === 'append-message' || event.type === 'user-message') {
      // Codec convention: TMessage.id == wire msg-id from meta.messageId.
      // The View's getRunByMsgId / regenerate / edit rely on this.
...
const apply = (tree: DefaultTree<TestEvent, TestProjection>, opts: ApplyOpts): void => {
  ...
  if (opts.msgId) h[HEADER_MSG_ID] = opts.msgId;
...
describe('DefaultView', () => {
  ...
  describe('flattenNodes and getMessages', () => {
    it('returns RunNode[] along the visible chain', () => {
      apply(tree, { runId: 'R1', msgId: 'm1', message: { id: 'a', content: 'first' }, serial: 's1' });
      apply(tree, {
        runId: 'R2',
        msgId: 'm2',
        parent: 'm1',
    ...
    it("getMessages concatenates each Run's codec.getMessages output", () => {
      apply(tree, { runId: 'R1', msgId: 'm1', message: { id: 'a', content: 'q1' }, serial: 's1' });
      apply(tree, {
        runId: 'R1',
        msgId: 'm2',
        parent: 'm1',
      ...
      // The codec convention rebinds each TMessage.id to the wire msgId.
    ...
    it('preserves per-Run order across many-message Runs', () => {
      ...
      apply(tree, { runId: 'R1', msgId: 'a', message: { id: 'a', content: 'a-1' }, serial: 's1' });
      apply(tree, { runId: 'R1', msgId: 'b', message: { id: 'b', content: 'a-2' }, serial: 's2' });
      apply(tree, { runId: 'R1', msgId: 'c', message: { id: 'c', content: 'a-3' }, serial: 's3' });
      apply(tree, {
        runId: 'R2',
        msgId: 'd',
        parent: 'c',
    ...
    it('flattens a five-turn linear conversation in publish order', () => {
      ...
      apply(tree, { runId: 'R1', msgId: 'u1', message: { id: 'u1', content: 'q1' }, serial: 's01' });
      apply(tree, { runId: 'R1', msgId: 'a1', parent: 'u1', message: { id: 'a1', content: 'r1' }, serial: 's02' });
      apply(tree, { runId: 'R2', msgId: 'u2', parent: 'a1', message: { id: 'u2', content: 'q2' }, serial: 's03' });
      apply(tree, { runId: 'R2', msgId: 'a2', parent: 'u2', message: { id: 'a2', content: 'r2' }, serial: 's04' });
      apply(tree, { runId: 'R3', msgId: 'u3', parent: 'a2', message: { id: 'u3', content: 'q3' }, serial: 's05' });
      apply(tree, { runId: 'R3', msgId: 'a3', parent: 'u3', message: { id: 'a3', content: 'r3' }, serial: 's06' });
      apply(tree, { runId: 'R4', msgId: 'u4', parent: 'a3', message: { id: 'u4', content: 'q4' }, serial: 's07' });
      apply(tree, { runId: 'R4', msgId: 'a4', parent: 'u4', message: { id: 'a4', content: 'r4' }, serial: 's08' });
      apply(tree, { runId: 'R5', msgId: 'u5', parent: 'a4', message: { id: 'u5', content: 'q5' }, serial: 's09' });
      apply(tree, { runId: 'R5', msgId: 'a5', parent: 'u5', message: { id: 'a5', content: 'r5' }, serial: 's10' });
  ...
  describe('query methods', () => {
    beforeEach(() => {
      apply(tree, { runId: 'R1', msgId: 'm1', message: { id: 'a', content: 'q' }, serial: 's1' });
      apply(tree, {
        runId: 'R2',
        msgId: 'm2',
        parent: 'm1',
    ...
    it('getRunByMsgId resolves the owning Run', () => {
      expect(view.getRunByMsgId('m1')?.runId).toBe('R1');
      expect(view.getRunByMsgId('m2')?.runId).toBe('R2');
      expect(view.getRunByMsgId('m-unknown')).toBeUndefined();
    });
  ...
  describe('branch selection', () => {
    ...
    it('getSelectedIndex returns 0 for an unforked Run', () => {
      apply(tree, { runId: 'R1', msgId: 'm1', message: { id: 'a', content: 'x' }, serial: 's1' });
  ...
  describe('write operations', () => {
    ...
    it('sendMessage forwards history (TMessage[]) and parentMsgId', async () => {
      apply(tree, { runId: 'R1', msgId: 'm1', message: { id: 'a', content: 'first' }, serial: 's1' });
      await view.sendMessage({ id: 'b', content: 'second' });

      const call = vi.mocked(sendDelegate).mock.calls[0];
      if (!call) throw new Error('expected delegate call');
      // The codec convention rebinds TMessage.id to the wire msgId.
      expect(call[2]).toEqual([{ id: 'm1', content: 'first' }]); // history
      expect(call[3]).toBe('m1'); // parentMsgId = last visible message's id (= wire msgId)
    });

    it('sendMessage with empty visible chain passes undefined parentMsgId', async () => {
    ...
    it('sendMessage uses view-local branch selection as history context', async () => {
      ...
      apply(tree, { runId: 'R1', msgId: 'u1', role: 'user', message: { id: 'a', content: 'q' }, serial: 's1' });
      apply(tree, {
        runId: 'R2',
        msgId: 'a1',
        parent: 'u1',
    ...
    it('regenerate produces a regenerate event keyed on the resolved parent and target msg-ids; sendOptions carry parent (no forkOf)', async () => {
      apply(tree, {
        runId: 'R1',
        msgId: 'u1',
        role: 'user',
      ...
      // Codec convention: TMessage.id is set to the wire msg-id at fold time.
    ...
    it('regenerate of an already-regenerated assistant resolves parent to the user prompt, not the hidden original assistant', async () => {
      ...
      // (continuation of R1, regeneratesMsgId=a1, owns a1p). The visible
      ...
      // The regenerate event's anchor msg-id must be the CANONICAL
      // anchor (a1), not the clicked-on regen content (a1p). Anchoring
      // every regen at the same canonical msg-id grows a single group
    ...
    it('regenerate throws when the target has no predecessor', async () => {
      apply(tree, { runId: 'R1', msgId: 'only', message: { id: 'x', content: 'x' }, serial: 's1' });
  ...
  describe('event scoping', () => {
    it('forwards update on tree structural change', () => {
      const handler = vi.fn();
      view.on('update', handler);
      apply(tree, { runId: 'R1', msgId: 'm1', message: { id: 'a', content: 'hi' }, serial: 's1' });
      expect(handler).toHaveBeenCalled();
    });
    ...
    it('forwards lifecycle / control ably-messages without a runId or msgId', () => {
      const handler = vi.fn();
      view.on('ably-message', handler);
  ...
  describe('reference stability', () => {
    ...
    it("suppresses 'update' when projection-updated arrives but projection and messages are unchanged", () => {
      ...
      apply(noopTree, { runId: 'R1', msgId: 'm1', message: { id: 'a', content: 'q' }, serial: 's1' });
      ...
      noopTree.applyMessage([{ type: 'append-message', message: { id: 'x', content: 'noop' } }], {
        [HEADER_RUN_ID]: 'R1',
        [HEADER_MSG_ID]: 'm-noop',
      });

      // structural emit on the new msgId index entry is allowed; the
  ...
  describe('multi-view', () => {
    ...
    it('branch selection is per-view (selecting in one does not affect the other)', () => {
      ...
      // elsewhere).
      apply(tree, { runId: 'R1', msgId: 'u1', message: { id: 'a', content: 'q' }, serial: 's1' });
      apply(tree, {
        runId: 'R2',
        msgId: 'a1',
        parent: 'u1',
  ...
  describe('loadOlder / hasOlder', () => {
      ...
      const items = [
        {
          ...
          headers: { [HEADER_RUN_ID]: 'R0', [HEADER_MSG_ID]: 'mh1' },
          serial: 's0',
        },
      ];
      const rawMsg = {
        name: 'fake',
        serial: 's0',
        extras: { headers: { [HEADER_RUN_ID]: 'R0', [HEADER_MSG_ID]: 'mh1' } },
      } as unknown as Ably.InboundMessage;
    ...
    it('withholds excess Runs and drains them on subsequent loadOlder calls without re-fetching', async () => {
      ...
      const items = [0, 1, 2].map((i) => ({
        ...
        headers: { [HEADER_RUN_ID]: `R${String(i)}`, [HEADER_MSG_ID]: `mh${String(i)}` },
        serial: `s${String(i)}`,
      }));
      const rawMessages = [0, 1, 2].map(
        (i) =>
          ({
            name: 'fake',
            serial: `s${String(i)}`,
            extras: { headers: { [HEADER_RUN_ID]: `R${String(i)}`, [HEADER_MSG_ID]: `mh${String(i)}` } },
          }) as unknown as Ably.InboundMessage,
      );
      codec.createDecoder = vi.fn(() => ({
        decode: (msg: Ably.InboundMessage) => {
          const id = (msg.extras as { headers: Record<string, string> }).headers[HEADER_MSG_ID] ?? 'unknown';
    ...
    it('handles a Run that spans multiple channel pages by carrying state across decodeHistory.next()', async () => {
      ...
      const headersA = { [HEADER_RUN_ID]: 'R-multi', [HEADER_MSG_ID]: 'm-multi-a' };
      const headersB = { [HEADER_RUN_ID]: 'R-multi', [HEADER_MSG_ID]: 'm-multi-b' };
      const rawA = {
        name: 'fake',
        serial: 's01',
      ...
      codec.createDecoder = vi.fn(() => ({
        decode: (msg: Ably.InboundMessage) => {
          const id = (msg.extras as { headers: Record<string, string> }).headers[HEADER_MSG_ID] ?? '?';
  ...
  describe('branch auto-select after fork', () => {
    ...
    it('regenerate sets a pending regenerate selection that resolves when the new Run arrives', async () => {
      ...
      // in R2 (anchored at msg-id a1), parented under R2 (the prior Run).
      apply(tree, {
        runId: 'R2new',
        msgId: 'a2',
        parent: 'a1',
    ...
    it('keeps an explicit regen selection when another external regen lands afterwards', () => {
      ...
      // The codec rebinds TMessage.id to the wire msg-id, so the visible
      // ids match the apply()'d msgIds.
    ...
    it('edit auto-selects the new sibling Run from optimisticMsgIds', async () => {
      vi.mocked(sendDelegate).mockResolvedValueOnce({
        stream: new ReadableStream(),
        runId: 'R2edit',
  ...
  describe('regenerate-as-continuation', () => {
    ...
    // asst1's msg-id; the View replaces asst1 with R2's content at
  ...
  // at msg-ids; the View surfaces per-bubble nav rather than per-Run nav so
  ...
  describe('msg-anchored branch nav', () => {
    describe('regenerate', () => {
      ...
      it('hasMessageSiblings is false for the user prompt msg-id (not an anchor)', () => {
        expect(view.hasMessageSiblings('u1')).toBe(false);
      });

      it('hasMessageSiblings is true for the regen anchor msg-id', () => {
        expect(view.hasMessageSiblings('a1')).toBe(true);
      });

      it('hasMessageSiblings is true for a regenerator Run content msg-id', () => {
        expect(view.hasMessageSiblings('a2')).toBe(true);
      });

      it('getMessageSiblings returns the regen group for an anchor msg-id', () => {
      ...
      it('getMessageSiblings returns [] for a non-anchor msg-id', () => {
        expect(view.getMessageSiblings('u1')).toEqual([]);
      });

      it('selectMessageSibling on the anchor msg-id switches the regen selection', () => {
      ...
      it('selectMessageSibling on a non-anchor msg-id is a no-op', () => {
    ...
    describe('edit', () => {
      ...
      it('hasMessageSiblings is true for the user prompt msg-id (edit anchor)', () => {
        expect(view.hasMessageSiblings('u2')).toBe(true);
      });

      it('hasMessageSiblings is false for the assistant msg-id (not an edit anchor)', () => {
      ...
      it('selectMessageSibling on the assistant msg-id is a no-op (assistant is not the edit anchor)', () => {
        const before = view.getMessages().map((m) => m.id);
        view.selectMessageSibling('a2', 0);
        expect(view.getMessages().map((m) => m.id)).toEqual(before);
      });
    ...
    // is ambiguous — the msg-id is the disambiguator.
    ...
    describe('regenerate then edit (R1 in both groups)', () => {
      ...
      it('hasMessageSiblings disambiguates by msg-id: user prompt anchors fork-of, assistant anchors regen', () => {
      ...
      it('selectMessageSibling on the assistant msg-id navigates the REGEN group, not the fork-of group', () => {
      ...
      it('selectMessageSibling on the user-prompt msg-id navigates the FORK-OF group', () => {
        expect(view.getMessages().map((m) => m.id)).toEqual(['u1', 'a1p']);
      ...
      it('getSelectedMessageSiblingIndex reports the correct group selection for each msg-id', () => {
