 ...
 * the public API, it reads as the wiring a developer would write to surface a
 * failure in their own UI, and it fails when the behaviour breaks. This is the
 * only integration file that mounts a real `useChat`, because what it asserts
 * is what `useChat` reports to the application.
 *
