 ...
 * the public API, it reads as the code a developer would write to encode and
 * decode a conversation, and it fails when the behaviour breaks. This one sits
 * below the transport tier, so it publishes and decodes without standing a
 * transport up at all.
 *
