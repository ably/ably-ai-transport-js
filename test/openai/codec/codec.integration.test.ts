...
describe('OpenAI ResponsesCodec integration', () => {
  ...
  it('multiple messages per run roundtrip (two codec-message-ids → two messages)', async () => {
    const channelName = uniqueChannelName('openai-multi-message-roundtrip');
    const pubChannel = ablyRealtimeClient().channels.get(channelName);
    const subChannel = ablyRealtimeClient().channels.get(channelName);
    ...
    // Two text streams arrive, one per message; resolve once both have ended.
    let textDoneCount = 0;
    await subChannel.subscribe((msg) => {
      const decoded = decoder.decode(msg);
      for (const event of toCodecEvents(decoded)) projection = ResponsesCodec.fold(projection, event, metaOf(msg));
    ...
    const first = ResponsesCodec.createEncoder(pubChannel, { onMessage: stampHeaders('run-1', 'msg-1') });
    ...
    const second = ResponsesCodec.createEncoder(pubChannel, { onMessage: stampHeaders('run-1', 'msg-2') });
    ...
    await second.publishOutput(textDone('item-2', 'second'));
    await second.publishOutput(completed());
    await second.close();
    ...
    const textOf = (message: (typeof messages)[number] | undefined): string => {
      const item = message?.message.items.find((i): i is Responses.ResponseOutputMessage => i.type === 'message');
