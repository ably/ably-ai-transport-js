...
// it proves the encoder itself slimmed the item, independently of the decoder:
// the decoder just reads the same `item` field back, and a decoded-and-folded
// assertion can't distinguish slim from un-slim (both fold to the same
// projection), so only the raw payload is the decode-independent check.
...
describe('OpenAI codec roundtrip (offline)', () => {
  ...
  it("folds output_item.done's message logprobs into the projected output_text part", async () => {
    ...
    // where it survives the slim with its real SDK type (so the fold is
    // cast-free). See slimDoneItem / the reducer's output_item.done handler.
  ...
  it('folds logprobs onto the right content slot in a multi-part (refusal + output_text) message', async () => {
    // The slim carries the message's parts index-aligned, so a logprobs-bearing
  ...
  it('decodes output_item.done for a message as a lean item (no content) when logprobs were not requested', async () => {
    // Wire economy: a reply without logprobs slims output_item.done to
  ...
  it('streams function-call arguments under the item id and folds them back', async () => {
    ...
    // start (fItem), not the slim output_item.done.
  ...
  it('preserves a reasoning item encrypted_content across the wire (store:false round-trip)', async () => {
    // encrypted_content is done-only and never streamed; seed the owner via
    // `added` WITHOUT it so this proves the slim output_item.done carries it, and
  ...
  it('throws on an unmodelled output item type at encode (added and done)', async () => {
    ...
    // model (and not a valid ResponseInputItem). The item envelope rides
...
// item on the wire is exactly the slim shape SlimDoneItem declares (see
// slimDoneItem / slimMessageContent in descriptors.ts); the first test covers
...
// the fields it then proves the wire strips. A slim assertion can't tell "the
...
// pass vacuously.
describe('OpenAI codec wire slimming', () => {
  it("publishes each item-carrying event's item under an `{ item }` data envelope", async () => {
    ...
    // Two things at once, via an exact toEqual. Slimming: each event is reduced
    ...
    // event itself is slimmed; the other tests slim output_item.done's item).
    // Framing: that item rides under a top-level `item` key rather than as the
    // bare payload, reserving room for a sibling wire field to join the envelope.
    // output_item.added carries the full item; output_item.done the slim item;
  ...
  it('drops sequence_number from every decoded output event', async () => {
    ...
    // codec's five streamed families (output_text, function_call_arguments,
  ...
  it('output_item.done for a message slims to id/type/status, with no content when logprobs is an empty array', async () => {
    ...
    // none ride the wire. An empty logprobs array is not logprobs either (the
    // length>0 guard in slimMessageContent), so there is no `content` at all —
  ...
  it('output_item.done for a message slims to per-part logprobs, index-aligned, dropping text/annotations/refusal string', async () => {
    const logprobs: Responses.ResponseOutputText['logprobs'] = [
      { token: 'Hi', logprob: -0.1, bytes: [72, 105], top_logprobs: [] },
    ];
    ...
    const { inbound } = await roundtrip([itemDone(item)]);
    // Only the logprobs residue rides the wire, in a placeholder array aligned
    // with the content: the refusal slims to just its type, the output_text to
  ...
  it('output_item.done for a function_call slims to id/type/status, dropping call_id/name/arguments', async () => {
  ...
  it('output_item.done for a reasoning item slims to id/type/status/encrypted_content, dropping summary', async () => {
  ...
  it('output_item.done for a reasoning item slims to id/type/status, omitting encrypted_content when the item has none', async () => {
    const reasoning: Responses.ResponseReasoningItem = {
      id: 'rs_1',
      type: 'reasoning',
      status: 'completed',
    ...
    // sent as an empty string (the string guard in slimDoneItem); summary is
