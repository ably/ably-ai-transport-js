...
describe('OpenAI decodeLifecycle (mid-stream join)', () => {
  ...
  it('introduces the owner once when sibling streams share an item on join', async () => {
    // One message with output_text at content[0] and a refusal at content[1] —
    // two streams, one owner. Both streams' joins must not double the item.
    ...
    // Only the first stream's join introduces the shared owner.
  ...
  it('does not double the owner for a client present at the genuine start', async () => {
    // A full replay (the real output_item.added decoded before the stream)
    // must not also synthesise the owner: exactly one message item survives.
