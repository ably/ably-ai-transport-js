...
export const decodedOf = (events: OpenAIEvent[]): OpenAIEvent[] =>
  events
    ...
    .map(
      (e) =>
        // CAST: the same member without `sequence_number`, which the codec never carries.
        Object.fromEntries(Object.entries(e).filter(([key]) => key !== 'sequence_number')) as OpenAIEvent,
    );
