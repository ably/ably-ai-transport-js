...
const rawMessage = (state: OpenAIProjection): Responses.ResponseOutputMessage | undefined =>
  state.entry?.message.items.find((i): i is Responses.ResponseOutputMessage => i.type === 'message');

const rawReasoningItem = (state: OpenAIProjection): Responses.ResponseReasoningItem | undefined =>
  state.entry?.message.items.find((i): i is Responses.ResponseReasoningItem => i.type === 'reasoning');
...
describe('OpenAI reducer', () => {
  ...
  describe('output_item.added', () => {
    it('is find-or-create by id: a repeated add does not double the item', () => {
      // The decode-lifecycle mid-stream-join repair can synthesise an owner add
      // that duplicates the real one. Two adds for one id fold to a single item,
      // and the deltas still accumulate into it.
      const state = foldOutputs([
        created(),
        itemAdded(messageItem('msg_1')),
        itemAdded(messageItem('msg_1')),
    ...
    it('dedups regardless of order (synthetic owner then real one)', () => {
      // On a reverse-order join (owner synthesised on the stream, then the real
    ...
    it('skips an item type the codec does not model', () => {
      ...
      // mirroring the decoder ignoring unknown event kinds. So it never enters the
      // turn, keeping every stored item a valid model input. The entry itself
      // still gets created (it only needs a codec-message-id, not a real item),
      // but with no items — getMessages filters that out.
      const state = foldOutputs([itemAdded(computerCallOutputItem())]);
      expect(state.entry?.message.items).toHaveLength(0);
      expect(getMessages(state)).toHaveLength(0);
    });
  });
  ...
  describe('output_item.done', () => {
    it('finalises the item status in place; content stays from the streams', () => {
      const state = foldOutputs([
        ...
        textDelta('msg_1', 'partial'),
        textDone('msg_1', 'the full text'),
        // The output_item.done envelope is wire-slim: its own content is ignored — only the
    ...
    it("folds each output_text part's logprobs into its content slot by index", () => {
      // The slim output_item.done item carries per-part logprobs index-aligned with the
    ...
    it('with no prior added is ignored (no owner to finalise)', () => {
      // The slim output_item.done finalises an existing owner's status; with no output_item.added there
    ...
    it('without an id is ignored', () => {
      // A slim output_item.done matches its owner by id; without one there is nothing to finalise.
    ...
    it("sets a reasoning item's encrypted_content (done-only, never streamed)", () => {
      ...
      // of chain-of-thought and is never streamed, so the slim output_item.done envelope must
      // still deliver it. Seed the owner via `added` WITHOUT the blob so this
  ...
  describe('leniency: unopened slots and orphan events', () => {
    ...
    // behaviour so a per-family regression to create-in-place can't slip through.
  ...
  describe('function calls', () => {
    ...
    it('folds a server-side function call and its output into one assistant turn', () => {
      ...
      // slim output_item.done finalises status); its arguments arrive on the stream. The agent
