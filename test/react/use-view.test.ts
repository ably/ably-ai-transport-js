...
describe('useView', () => {
  ...
  describe('Run-keyed query callbacks', () => {
    ...
    it('getRunByMsgId forwards to view.getRunByMsgId', () => {
      const mock = createMockSession();
      const node = makeRun('run-1');
      (mock.view.getRunByMsgId as ReturnType<typeof vi.fn>).mockReturnValue(node);
      const { result } = renderHook(() => useView({ session: mock.session }));
      expect(result.current.getRunByMsgId('m1')).toBe(node);
    });
    ...
    it('safe defaults when no session is available', () => {
      ...
      expect(result.current.getRunByMsgId('a')).toBeUndefined();
