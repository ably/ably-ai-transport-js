 ...
 * tool resolution, which opens a new run — plus a reload that reads history
...
describe('ChatTransport drives useChat with no external message state', () => {
  ...
  it('a reload reads history and resumes the run still in flight', async () => {
    const fake = new FakeClientTransport();
    fake.historyBatches = [
      {
        events: [
    ...
    // The application's hydration: read, hand the finished messages to
    // setMessages, then resume. The history read withholds a1 because run-1 has not
