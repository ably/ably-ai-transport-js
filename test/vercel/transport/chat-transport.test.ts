 ...
 * path it merges history into messages and retains an unfinished run's
 ...
 * read and the reconnect that pairs with it, step supersede, and the terminal
...
describe('ChatTransport', () => {
  ...
  describe('fresh send', () => {
    ...
    it('refuses to read once closed', async () => {
      const { chat } = setup();
      chat.close();
  ...
  describe('regeneration', () => {
    it('anchors on the last message useChat left in its truncated list', async () => {
      stubChatFetch();
      const { fake, chat } = setup('run-3');

      // useChat drops the assistant message it is regenerating before it asks,
      // so the list ends with the user turn that prompted the reply.
      await chat.sendMessages(
        sendOptions([userMessage('u1', 'hi'), userMessage('u2', 'again')], {
      ...
      expect(fake.published).toHaveLength(1);
      expect(fake.published[0]?.event).toEqual({ kind: 'regenerate', payload: { messageId: 'u2' } });
      expect(fake.published[0]?.opts?.transportMessageId).toBeTypeOf('string');
    });

    it('anchors without useChat naming a message', async () => {
      stubChatFetch();
      const { fake, chat } = setup('run-3');

      // `chat.regenerate()` with no argument reaches the adapter with
      // `messageId: undefined`, which used to be rejected. The truncated list
      // is enough on its own.
      await chat.sendMessages(sendOptions([userMessage('u1', 'hi')], { trigger: 'regenerate-message' }));

      expect(fake.published[0]?.event).toEqual({ kind: 'regenerate', payload: { messageId: 'u1' } });
    });

    it('rejects when the message list is empty', async () => {
      stubChatFetch();
      const { chat } = setup('run-3');

      await expect(chat.sendMessages(sendOptions([], { trigger: 'regenerate-message' }))).rejects.toBeErrorInfoWithCode(
  ...
  describe('readSince', () => {
    it('returns the history messages, oldest first', async () => {
      const { fake, chat } = setup();
      fake.historyBatches = [
        {
          events: [
    ...
    it('reports exhausted false when the history read stopped at the serial', async () => {
      const { fake, chat } = setup();
      fake.historyBatches = [
        {
    ...
    it('keeps every part of a message the codec exploded across wire events', async () => {
      ...
      expect(messages[0]?.events.map((event) => event.event)).toEqual([
        { kind: 'message', payload: { id: 'u1', role: 'user', parts: [{ type: 'text', text: 'look at ' }] } },
        { kind: 'message', payload: { id: 'u1', role: 'user', parts: [{ type: 'text', text: 'this' }] } },
      ]);
    });

    it('drops a superseded step attempt from the history read', async () => {
      const { fake, chat } = setup();
      fake.historyBatches = [
        {
          events: [
  ...
  describe('history tool resolutions', () => {
  ...
  describe('grouping the history read', () => {
    it('joins two publishes of one assistant message on the id their start chunks carry', async () => {
      ...
      expect(messages[0]?.events.flatMap((event) => (event.direction === 'output' ? [event.event.type] : []))).toEqual([
        'start',
    ...
    it('keeps an input naming a message this read never saw', async () => {
  ...
  describe('a history message that names no id', () => {
      ...
      fake.historyBatches = [
        {
          events: [
            ...
            // business; the history read reports what was published either way.
  ...
  describe('a second readSince', () => {
    it('still reports the conversation the first read paged', async () => {
      const { fake, chat } = setup();
      fake.historyBatches = [
        {
          events: [
      ...
      // The pager opens its cursor once and keeps it, so a second read fetches
    ...
    it('keeps a withheld message when the run ends live between the reads', async () => {
      const { fake, chat } = setup();
      fake.historyBatches = [
        {
          events: [
      ...
      // The run's end arrives on the channel, which is the only way a history read
    ...
    it('keeps the terminal in the retention when the live buffer has trimmed past it', async () => {
      const { fake, chat } = setup();
      fake.historyBatches = [
        {
          events: [
      ...
      fake.emit(runEndEvent('run-1'));
      // Push the terminal out of the live buffer, so the retention is the only
      // place left holding it. One event per LIVE_BUFFER_LIMIT slot is enough:
      // the terminal is the oldest entry and the first eviction takes it.
      for (let i = 0; i < 2000; i++) {
        const slot = String(i);
        fake.emit(outputEvent('run-filler', `wire-f${slot}`, [], `serial-f${slot}`));
      }
      // The second read re-reads the same pages, which still show run-1 open. It
      // must merge them into the retention rather than rebuild it, or the
      // resumed stream has nothing to close on and useChat hangs on streaming.
      await chat.readSince();
    ...
    it('keeps a live supersede’s events for the repair', async () => {
      stubChatFetch();
      const { fake, chat } = setup('run-1');
      // An exhausted, empty read: run-1 exists only live, so no page mentions
      // it and no history read can report it either open or ended.
      await chat.readSince();
      ...
      // A second read in the middle of the repair. It says nothing about run-1,
      // and silence is no release: dropping the retention here would leave the
      // consumer's resume with no run to address.
      await chat.readSince();
    ...
    it('delivers an event once when a second read overlaps the live buffer', async () => {
      const { fake, chat } = setup();
  ...
  describe('reconnectToStream', () => {
    it('returns null before the history read has run', async () => {
      const { chat } = setup();
      // eslint-disable-next-line unicorn/no-null -- the SDK contract is null
      expect(await chat.reconnectToStream({ chatId: 'ai:test' })).toBe(null);
    });

    it('returns null when the history read found no open run', async () => {
      const { fake, chat } = setup();
      fake.historyBatches = [
        {
    ...
    it('replays the withheld message, then goes live', async () => {
      ...
      // unchanged from the history delivery and only the version advances.
      fake.emit(
        appendEvent(
          'run-2',
          'wire-a2',
    ...
    it('replays every append buffered during the history read, not just the first', async () => {
      const { fake, chat } = setup();
      ...
      // live buffer, and the history read brings back only the run's start, because
    ...
    it('still drops a delivery the history read and the live buffer both hold', async () => {
      const { fake, chat } = setup();
      ...
      // can land there and come back from the history read as well. It must reach the
      // reducer once.
      fake.emit(opener);
    ...
    it('closes cleanly when the run ended between the history read and the reconnect', async () => {
      const { fake, chat } = setup();
      fake.historyBatches = [
        {
          events: [
      ...
      const historyRead = chat.readSince();
      // The end lands in the history read's live buffer, because readSince subscribed
      // before it paged.
      fake.emit(runEndEvent('run-2'));
      await historyRead;
  ...
  describe('resume discovery', () => {
    it('goes live on a run another participant started after the history read', async () => {
      const { fake, chat } = setup();
      fake.historyBatches = [
        {
          events: [outputEvent('run-1', 'wire-a1', assistantChunks('a1', 'done'), 'serial-11'), runEndEvent('run-1')],
    ...
    it('still delivers a withheld message whose run ended before the reconnect', async () => {
      ...
      // run-a's message was withheld from the history read, so the stream is its only
    ...
    it('keeps live output that arrived before a run ended behind the history read', async () => {
      const { fake, chat } = setup();
      fake.historyBatches = [
        {
          events: [
    ...
    it('buffers events published before the history read runs', async () => {
      const { fake, chat } = setup();
  ...
  describe('onForeignRun', () => {
    ...
    describe('on going idle', () => {
      // The announcement is deferred to a task of its own, so a subscriber
      // that answers with resumeStream cannot race the resume already in
      // flight. Fake timers drive that task rather than waiting on a clock.
      beforeEach(() => {
        vi.useFakeTimers();
      });
      ...
      it('offers a run it passed over while streaming', async () => {
        const { fake, chat } = setup();
        fake.historyBatches = [
          {
            events: [
        ...
        // run-2 starts while the resumed run-1 stream is open, so the live
        // announcement passes it over.
        fake.emit(runStartEvent('run-2', 'serial-20'));
        ...
        // Idle again with run-2 still going: without this the run stays
        // invisible until the application hydrates afresh.
        expect(seen).toEqual(['run-2']);
      });

      it('says nothing once every run it saw has ended', async () => {
        stubChatFetch();
        const { fake, chat } = setup('run-1');
        const seen: string[] = [];
        chat.onForeignRun((runId) => seen.push(runId));
        ...
        await Promise.resolve();
        fake.emit(runStartEvent('run-1', 'serial-10'));
        fake.emit(outputEvent('run-1', 'wire-a1', assistantChunks('a1', 'hi'), 'serial-11'));
        fake.emit(runEndEvent('run-1'));
        await readAll(stream);
        await vi.advanceTimersByTimeAsync(0);
