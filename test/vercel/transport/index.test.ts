...
describe('Vercel createAgentTransport', () => {
  it('creates a connectable agent transport with the codec pre-bound', async () => {
    ...
    // A pinned runId alone opens a fresh run; `continuation: true` is what
    // makes the open a re-entry publishing `ai-run-resume`.
    const run = transport.openRun({ runId: 'run-1' });
    await run.end({ reason: 'complete' });

    expect(channel.publishCalls.some((m) => m.name === 'ai-run-start')).toBe(true);
    transport.close();
  });
});
